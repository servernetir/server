#!/usr/bin/env bash
#
# مالی مرحلهٔ ۱ — کارمزدِ درگاه، قطعِ زمانی، ارزهای غیرِ پایه. شهریور ۱۴۰۵.
#
# اجرا از ترمینال cPanel (اکانت servernetcloud):
#   DRY=1 bash <(curl -fsSL https://raw.githubusercontent.com/servernetir/server/develop/scripts/deploy-finance-phase1.sh)
#   bash <(curl -fsSL https://raw.githubusercontent.com/servernetir/server/develop/scripts/deploy-finance-phase1.sh) [<SHA>]
#
# چه چیزی دیپلوی می‌شود:
#   · ZarinPalGateway — کارمزدِ برگشتی از ریال به تومان تبدیل می‌شود
#   · BusinessLedger — کارمزد به‌عنوان هزینه ثبت می‌شود؛ occurred_at به وقتِ
#     تهران؛ ردیف‌های غیرتومانی جدا شمرده می‌شوند (by_currency)
#   · finance.blade.php — نمایشِ همان ردیف‌های ارزی
#
# 🔴 چرا لازم شد: `payments.fee` پر می‌شد و هیچ‌جا خوانده نمی‌شد، پس سودِ
#    گزارش‌شده به اندازهٔ کلِ کارمزدِ سالانه بیش‌برآورد بود — و چون کارمزد
#    هزینهٔ قابلِ قبولِ مالیاتی است، مالیاتِ بیشتری هم بابتش داده می‌شد.
#
# ⚠️ مهاجرت ندارد. فایلِ استاتیک ندارد. routes را دست نمی‌زند.
#
# ⚠️ ترتیب معنادار است: اول درگاه (تبدیلِ واحد)، بعد دفتر (که آن عدد را
#    هزینه می‌کند). برعکسش یعنی پنجره‌ای که دفتر عددِ **ریالی** را به‌عنوان
#    تومان هزینه می‌کند ⇒ کارمزدِ ده‌برابر در دفتر.
#
# ⚠️ ویو عمداً آخر است. `@if(! empty($s['by_currency']))` روی کلیدِ نبود هم
#    امن است، پس ویوِ تازه با دفترِ کهنه نمی‌شکند — ولی ترتیب باز هم درست
#    نگه داشته می‌شود تا حالتِ میانی «قابلیت هنوز نیست» باشد.
#
# منطق عیناً از scripts/deploy-redact-cards.sh: merge سه‌طرفه با پایهٔ
...
WORK="$HOME/deploy-finance-phase1"
...
APP_FILES="
app/Services/Payment/ZarinPalGateway.php
...
need_file "$APP/app/Services/Payment/ZarinPalGateway.php"
need_file "$APP/app/Services/Finance/BusinessLedger.php"
need_file "$APP/resources/views/admin/finance.blade.php"
...
#
# اگر دفتر بنشیند و درگاه ننشیند، کارمزد **ده برابر** هزینه می‌شود: عددِ
# ریالی در ستونی که واحدش تومان است. هیچ خطایی هم نمی‌دهد — فقط سود اشتباه.
g app/Services/Payment/ZarinPalGateway.php "private function toToman"
g app/Services/Payment/ZarinPalGateway.php "toToman((int) data_get"

g app/Services/Finance/BusinessLedger.php "private function recordGatewayFee"
g app/Services/Finance/BusinessLedger.php "category: 'payment_fee'"
g app/Services/Finance/BusinessLedger.php "display_timezone"
g app/Services/Finance/BusinessLedger.php "'by_currency'"
g resources/views/admin/finance.blade.php "by_currency"

# ── ⚠️ کارِ دیگران که این merge نباید برش دارد ──
#
# BusinessLedger پرکار است: چهار متدِ ثبتِ دیگر رویش نشسته که هیچ‌کدام
# خطا نمی‌دهند اگر بی‌صدا برداشته شوند — فقط درآمد و هزینه گم می‌شود.
g app/Services/Finance/BusinessLedger.php "public function recordCreditSale"
g app/Services/Finance/BusinessLedger.php "public function recordInvoiceRefund"
g app/Services/Finance/BusinessLedger.php "public function recordDomainWholesale"
g app/Services/Finance/BusinessLedger.php "public function recordServerRent"
g app/Services/Finance/BusinessLedger.php "const CATEGORY_LABELS"

g app/Services/Payment/ZarinPalGateway.php "private function toRial"
g app/Services/Payment/ZarinPalGateway.php "private function explain"
g app/Services/Payment/ZarinPalGateway.php "private function readError"

# ویو هم دست‌خورده است: بخشِ دلایلِ ترکِ مشتری و فرمِ ثبتِ دستی رویش نشسته.
g resources/views/admin/finance.blade.php "تفکیک هزینه"
g resources/views/admin/finance.blade.php "ثبت دستی"
g resources/views/admin/finance.blade.php "درصدها نسبت به پاسخ‌های داده‌شده"

# 🔴 نگاشتِ $kindLabel از پایینِ فایل به بالا منتقل شده. اگر merge نسخهٔ
#    کهنه را نگه دارد، **دو** تعریف می‌ماند و آخری برنده می‌شود — بی‌خطا،
#    ولی جدولِ ردیف‌های اخیر و بخشِ ارزی دو نامِ متفاوت می‌گویند.
KL=$(grep -c 'kindLabel = \[' "$APP/resources/views/admin/finance.blade.php" 2>/dev/null || echo 0)
echo "═══ تعریفِ kindLabel: $KL (باید دقیقاً ۱ باشد) ═══"
if [ "$KL" -ne 1 ]; then
  echo "🔴 نگاشتِ kindLabel $KL بار تعریف شده — merge نسخهٔ کهنه را نگه داشته."
  union_ok=0
else
  echo "✅ یک تعریف"
fi
...
echo "  ۱. یک پرداختِ واقعیِ تازه از زرین‌پال بگیرید."
echo "  ۲. /admin/finance → «تفکیک هزینه» باید ردیفِ «کارمزد درگاه» داشته باشد."
echo "  ۳. عددش باید تومان باشد نه ریال — روی تراکنشِ ۱۰۰٬۰۰۰ تومانی چیزی"
echo "     حدودِ هزار تومان، نه ده‌هزار."
echo
echo "═══ چه چیزی این دیپلوی درست نمی‌کند ═══"
echo "  · ردیف‌های کهنهٔ payments.fee به ریال‌اند و دست‌نخورده می‌مانند."
echo "    هیچ ردیفِ دفتری از آن‌ها ساخته نمی‌شود، پس بی‌خطرند — ولی جمعِ"
echo "    تاریخیِ آن ستون واحدهای قاطی دارد."
echo "  · کارمزدِ پرداخت‌های **گذشته** ثبت نمی‌شود: دفتر فقط در لحظهٔ تسویه"
echo "    می‌نویسد و آن پرداخت‌ها قبلاً تسویه شده‌اند."
echo "  · درآمدِ سرورِ ساعتی هنوز وارد دفتر نمی‌شود — عمدی، مرحلهٔ بعد."
