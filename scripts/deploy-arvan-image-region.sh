...
grep -qF "function imageForRegion" "$dest" || { echo "🔴 تطبیقِ ایمیجِ منطقه‌ای ننشست"; ok=0; }
grep -qF "distributions" "$dest" || { echo "🔴 پارامترِ distributions نیست"; ok=0; }
grep -qF "real_name" "$dest" || { echo "🔴 استفاده از real_name ننشست"; ok=0; }
