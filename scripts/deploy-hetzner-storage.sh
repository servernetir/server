...
# کشِ بهای تمام‌شده که کف از آن می‌خواند
g app/Console/Commands/HetznerStorageCatalog.php "HetznerStorageCosts::remember"
# محافظِ «دو بار نخر»
...
echo
echo "═══ این دیپلوی خودش چیزی را روشن یا خاموش نمی‌کند ═══"
echo "  فقط کد را جابه‌جا می‌کند. اینکه کدام پکیج فروخته می‌شود در پنل تعیین"
echo "  می‌شود (/admin/products) و این اسکریپت دستش نمی‌زند."
echo
echo "  ⚠️ نگاشتِ hetzner_storage.plans عمداً فقط سه پلن دارد: BK-1T/5T/10T."
echo "     کوچک‌ترین باکسِ هتزنر ۱ ترابایت است، پس BK-100 و BK-500 زیرِ بهای"
echo "     تمام‌شده می‌افتند؛ و پلن‌های دانلود اصلاً قابلِ تحویل نیستند چون"
echo "     Storage Box دانلودِ عمومیِ HTTP ندارد."
