...
need_grep config/catalog/vps.php   "'discord' => ["
need_grep config/catalog/vps.php   'هاست ربات دیسکورد'
need_grep config/blog.php          "'post_products' =>"
need_grep config/blog.php          'iranian-discord-servers'
need_grep config/servernet.php     "'slug' => 'discord'"
need_grep app/helpers.php          'blog.post_products.'
need_grep app/Services/AiContent.php 'blog_related_product($brief'
need_grep resources/views/pages/blog-post.blade.php "\$post['slug'] ?? null"
...
check "https://servernet.cloud/vps/trading"                   "200"
...
echo
echo "═══ ۵) صفحهٔ تازه (گزارشی — تا ریستِ opcache ممکن است ۴۰۴ بدهد) ═══"
for p in /vps/discord /en/vps/discord /tr/vps/discord; do
  c=$(curl -s -o /dev/null -w '%{http_code}' --max-time 25 "https://servernet.cloud$p?qa=$STAMP")
  echo "  $([ "$c" = 200 ] && echo ✅ || echo ⚠️) $c  $p"
done
curl -s --max-time 25 "https://servernet.cloud/blog/iranian-discord-servers?qa=$STAMP" | grep -q '/vps/discord' \
  && echo "  ✅ پستِ دیسکورد به صفحهٔ محصول لینک می‌دهد" \
  || echo "  ⚠️ لینکِ پست→محصول هنوز دیده نمی‌شود (opcache؟)"
