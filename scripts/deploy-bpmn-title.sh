...
# ── کارِ این دیپلوی: عنوان باید عوض شده باشد و برند از داخلش رفته باشد
need_grep config/solutions.php 'طراح آنلاین و رایگان BPMN 2.0 + پلتفرم BPMS فارسی'
need_grep config/solutions.php 'Free Online BPMN 2.0 Designer + Enterprise BPMS'
need_grep config/solutions.php 'Ücretsiz Çevrimiçi BPMN 2.0 Tasarımcısı + BPMS'

# 🔴 ادعای منفی: عنوانِ کهنه نباید مانده باشد. اگر merge نیمه‌کاره بنشیند،
# فایل نوشته می‌شود و هیچ خطایی هم نمی‌دهد — فقط عنوان همان قبلی می‌مانَد.
if grep -qF -- 'طراح BPMN و پلتفرم مدیریت فرایند سازمانی (BPMS) سرورنت' "$APP/config/solutions.php"; then
  echo "🔴 عنوانِ کهنه هنوز در config/solutions.php هست"; union_ok=0
fi

# ⚠️ کارِ دیگران در همین فایل که merge نباید برش دارد
need_grep config/solutions.php "'bpmn-erp' => ["
need_grep config/solutions.php 'سرورنت ریموت'
need_grep config/solutions.php 'تلفن ابری سرورنت'
need_grep config/solutions.php 'https://bpmn.servernet.cloud'
...
check "https://servernet.cloud/solutions/bpmn-designer"   "200"
...
echo
echo "═══ ۵) عنوانِ واقعی روی سایت (تا ریستِ opcache ممکن است کهنه باشد) ═══"
for p in /solutions/bpmn-designer /en/solutions/bpmn-designer /tr/solutions/bpmn-designer; do
  t=$(curl -s --max-time 25 "https://servernet.cloud$p?qa=$STAMP" | grep -oE '<title>[^<]*</title>' | head -1)
  echo "  $p"
  echo "      $t"
done
echo "═══ خلاصه ═══"
