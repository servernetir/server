...
#
# `routes/web.php` را چند اسکریپت می‌برند. تازه‌ترینشان
# `deploy-licence-lifecycle.sh` بود (پین 6132a36) که جدِ همین کامیت است؛
# پینش این‌جا هم‌گرا شد تا ترتیبِ اجرا مهم نباشد.
#
# `config/servernet.php` را `deploy-gpu.sh` هم می‌برد (پین c3508a0، جدِ این
# کامیت). چون از آن پین به بعد آن اسکریپت این فایل را عوض نکرده، پایه و
# «نسخهٔ جدید»ش یکی می‌شوند ⇒ diff خالی ⇒ نسخهٔ سرور حفظ می‌شود.
#
