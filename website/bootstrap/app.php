...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withMiddleware(function (Middleware $middleware): void {
        ...
        $middleware->validateCsrfTokens(except: [
            'api/sms/*',
            /*
             * اسنپ‌پی نتیجهٔ پرداخت را با POST از **سرورِ خودش** می‌فرستد، پس
             * نشست و توکنِ CSRF ندارد.
             *
             * 🔴 جایِ CSRF را چیزِ قوی‌تری گرفته: هیچ‌چیزِ این فرم حکم نیست.
             * `transactionId` فقط برای پیدا کردنِ ردیف است و اینکه پول واقعاً
             * آمده یا نه، با تماسِ سرور-به-سرورِ verify تعیین می‌شود. پس حتی
             * فرمِ کاملاً جعلی هم چیزی را «پرداخت‌شده» نمی‌کند.
             */
            'payment/callback/*',
            ...
            'system/opcache',
            /*
            | 🔴 عاملِ هاستِ ایران یک اسکریپتِ curl است، نه مرورگر: نه نشست
            | دارد نه توکنِ CSRF. محافظش `X-Agent-Token` در
            | `PullController::authorizeAgent()` است.
            |
            | بی‌این خط، `POST /agent/guestpolicy/ack` **۴۱۹** می‌گرفت — و
            | بدترین بخشش این بود که خرابی کاملاً بی‌صدا می‌مانْد: عامل قواعد
            | را درست اعمال می‌کرد، تأییدش رد می‌شد، و پنل تا ابد «در انتظار»
            | نشان می‌داد. یعنی همان تمایزِ «ضربان» و «اعمال شد» که این کار
            | برای ساختنش نوشته شد، از درِ پشتی خراب می‌شد.
            |
            | ⚠️ هیچ تستی نمی‌توانست بگیردش: `PreventRequestForgery` در
            | `runningUnitTests()` کاملاً دور زده می‌شود، پس POSTِ بی‌توکن در
            | تست همیشه سبز است. گاردش در `AgentPullTest` روی **پیکربندی**
            | ادعا می‌کند، نه روی رفتار.
            */
            'agent/*',
        ]);
