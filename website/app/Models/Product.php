...
class Product extends Model
{
    ...
    public function priceForCycle(string $cycle, ?string $country = null): int
    {
        ...
        $price = price_toman((int) round($raw));

        /*
        | 🔴 کفِ حاشیه — «هرگز زیرِ قیمتِ خرید نفروش».
        |
        | این‌جا و نه جای دیگر، چون ضرر دقیقاً از همین خط می‌آمد: تخفیفِ دوره
        | (سالانه ۲۰٪) از کلِ حاشیهٔ یک محصولِ بازفروشی بزرگ‌تر بود. ماهانه سود
        | می‌داد و سالانه ۱۳٪ ضرر — و چون تحویل موفق می‌شد، هیچ خطایی هیچ‌جا
        | ثبت نمی‌شد.
        |
        | اصلاحِ عددها کافی نبود: نرخِ ارز حرکت می‌کند، درصدِ تخفیف ویرایش‌پذیر
        | است، و پلنِ تازه هم اضافه می‌شود. پس کف **محاسبه** می‌شود.
        |
        | ⚠️ صفر یعنی «کفی نداریم» (پلنِ غیرِ هتزنر، یا کشِ بهای خالی) نه
