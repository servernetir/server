...
class CloudInstance extends Model
{
    ...
    */
    ...
    /*
    |---------------------------------------------------------------------------
    | «از کدام خروج» — سه حالتِ ماشین
    |---------------------------------------------------------------------------
    |
    | کشور می‌گوید ترافیک از **کجای دنیا** بیرون برود؛ این می‌گوید از **کدام
    | مسیر**. دو ماشین می‌توانند هر دو `cc=de` باشند و یکی از استخرِ رایگان
    | برود و دیگری از سرورِ خودمان — پس این دو مفهومِ جدا هستند و نباید در یک
    | ستون قاطی شوند.
    |
    |   بی‌اکسیت      exitCountryCode() === null      ⇒ آی‌پیِ خودِ سرور (ایران)
    |   استخرِ رایگان  via = 'pool'                    ⇒ نودهای گیت‌هابیِ آن کشور
    |   آپ‌استریمِ من  via = 'u<id>'                    ⇒ سرور/کانفیگِ ثبت‌شدهٔ خودمان
    |
    | ⚠️ `via` بی‌معنی است وقتی اکسیت خاموش است. هر خواننده‌ای اول باید
    | `exitCountryCode()` را بپرسد؛ این متد عمداً برای ماشینِ بی‌اکسیت هم
    | `'pool'` برمی‌گرداند تا هیچ‌جا نال‌چک از قلم نیفتد و رفتار پیش‌فرض
    | «استخر» بمانَد.
    */

    /** مسیرِ خروج: `pool` (استخرِ رایگان) یا `u<id>` (آپ‌استریمِ ثبت‌شدهٔ خودمان). */
    public function exitVia(): string
    {
        $id = $this->exitUpstreamId();

        return $id === null ? 'pool' : ('u'.$id);
    }

    /**
     * شناسهٔ `ExitUpstream`ی که این ماشین به آن پین شده، یا `null` یعنی استخر.
     *
     * ⚠️ عمداً وجودِ خودِ ردیف را **نمی‌سنجد**: این متد در مسیرِ داغِ ایجنت صدا
     * زده می‌شود و یک کوئری به‌ازای هر ماشین یعنی N+1. اعتبارسنجی سرِ ثبت
     * انجام می‌شود؛ اگر آپ‌استریم بعداً حذف شود، هاست آن `via` را نمی‌شناسد و
     * طبقِ قاعدهٔ fail-closed چیزی اعمال نمی‌کند — نه اینکه بی‌صدا به استخر
     * برگردد و مشتری روی خروجی بنشیند که نخریده.
     */
    public function exitUpstreamId(): ?int
    {
        $raw = ($this->meta ?? [])['exit_via'] ?? null;

        if (! is_string($raw) || ! preg_match('/^u(\d+)$/', trim($raw), $m)) {
            return null;
        }
    ...
    /** آیا این ماشین به یک آپ‌استریمِ مشخص پین شده (نه استخرِ رایگان)؟ */
    public function isPinnedToUpstream(): bool
    {
        return $this->exitUpstreamId() !== null;
    }
    ...
    |--------------------------------------------------------------------------
    |
    | همهٔ مهمان‌ها روی یک LANِ مشترک (`vmbr1`, 10.10.10.0/24) نشسته‌اند، پس
    | ماشینِ یک مشتری به‌صورتِ پیش‌فرض ماشینِ مشتریِ دیگر و سرویس‌های میزبان را
    | می‌بیند. این کلید «حالتِ مطلوب» را نگه می‌دارد؛ اعمالش کارِ عاملِ میزبان است.
    |
    | 🔴 پیش‌فرض **true** است و عمداً: امروز همه دسترسی دارند. اگر نبودِ کلید را
    | «ببند» معنا کنیم، اولین دیپلوی هر مهمانی را که هرگز از پنل تنظیم نشده از
    | شبکهٔ داخلی می‌بُرد — از جمله ماشین‌های زیرساختیِ خودمان.
    */

    /** آیا این مهمان اجازهٔ دیدنِ شبکهٔ داخلی را دارد؟ (پیش‌فرض: بله) */
    public function lanAccess(): bool
    {
        $meta = $this->meta ?? [];

        if (! array_key_exists('lan_access', $meta)) {
            return true;
        }

        return (bool) $meta['lan_access'];
    }

    /** آیا این مقدار صریحاً از پنل تنظیم شده یا پیش‌فرض است؟ */
    public function lanAccessIsExplicit(): bool
    {
        return array_key_exists('lan_access', $this->meta ?? []);
    }
}
