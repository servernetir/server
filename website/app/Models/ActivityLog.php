...
class ActivityLog extends Model
{
    ...
    /**
     * کنش‌هایی که **فقط تیمِ مدیریت** می‌بیند.
     *
     * 🔴 «مدیر وارد پنلِ این مشتری شد» رویدادِ ماست، نه رویدادِ او. نشان دادنش
     * به مشتری هیچ سودی ندارد و دو زیان دارد: نگرانی («چرا وارد حسابم شدند؟»)
     * و افشای الگوی کارِ داخلی. کارفرما صریح خواست پنهان بماند.
     *
     * ⚠️ فهرست بر اساس **کنش** است نه `actor`. کنشِ کارمندی‌ای که مشتری باید
     * ببیند کم نیست — «مدیر رسیدِ واریزت را تأیید کرد» دقیقاً همان چیزی است که
     * باید ببیند. حذفِ کلِ `actor='staff'` آن‌ها را هم می‌بلعید.
     */
    public const STAFF_ONLY_ACTIONS = ['impersonate'];

    /**
     * لاگِ قابلِ نمایش به **خودِ مشتری**.
     *
     * ⚠️ اسکوپ است نه شرطِ درون‌خطی در کنترلر. نقطهٔ نمایشِ دوم روزی اضافه
     * می‌شود و اگر قاعده در کنترلر باشد، همان‌جا جا می‌افتد — و نشتی‌ای که
     * هیچ خطایی نمی‌سازد ماه‌ها دیده نمی‌شود.
     */
    public function scopeVisibleToCustomer($q)
    {
        return $q->whereNotIn('action', self::STAFF_ONLY_ACTIONS);
    }
    /**
