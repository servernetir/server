...
class CloudHourlyAudit extends Command
{
    ...
    public function handle(): int
    {
        ...
        */
        ...
        |
        | این فرمان هر روز ۰۶:۰۵ می‌دود و تا وقتی سرویسِ #۹۹ زیرِ بها باشد
        | کدِ ۱ می‌داد. زمان‌بند هر کدِ غیرِصفر را یک استثنا گزارش می‌کند، پس
        | سه روزِ پیاپی در `/admin/errors` ردیفِ **۵۰۰** نشست — کنارِ خطاهای
        | واقعی و به همان قرمزی.
        |
        | بهایش بیشتر از شلوغی است: در فهرستی که هر روز یک قرمزِ بی‌معنا دارد،
        | قرمزِ **واقعی** هم دیده نمی‌شود. همان درسِ ثبت‌شدهٔ «قرمزِ تصادفی یاد
        | می‌دهد قرمز را نادیده بگیرند».
        |
        | ⚠️ یافته گم نمی‌شود: با `--notify` همان لحظه به بله و ایمیلِ مدیر
        | رفته و در ردیابِ خطا هم `noteOnce` خورده — یعنی **رسیده**. پس در
        | حالتِ زمان‌بندی‌شده کدِ خروجی فقط می‌گوید «درست دویدم».
        |
        | ⚠️ بدونِ `--notify` (اجرای دستی یا CI) رفتار دست‌نخورده می‌مانَد: آن‌جا
        | هیچ اعلانی نمی‌رود و کدِ خروجی تنها راهِ فهمیدنِ نتیجه است.
        */
        if ($underwater !== [] && ! $this->option('notify')) {
            return self::FAILURE;
        }
