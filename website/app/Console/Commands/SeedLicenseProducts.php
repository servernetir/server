...
class SeedLicenseProducts extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (! Schema::hasColumn('products', 'requires_server_ip')) {
            // بی‌این ستون، پکیج ساخته می‌شود ولی فرمِ سفارش IP نمی‌پرسد و
            // اپراتور سفارشی می‌گیرد که نمی‌داند روی چه سروری فعالش کند.
            //
            // ⚠️ `FAILURE` عمدی است و نه استثنا: روتِ migrate/seed پروداکشن
            // چند فرمان را پشتِ سرِ هم می‌دواند و یک استثنا بقیه را می‌کشد.
            $this->warn('ستونِ requires_server_ip نیست — اول مهاجرت را بزنید، بعد دوباره همین فرمان را.');
        ...
        foreach ($plans as $i => $plan) {
            ...
            /*
            | ⚠️ اسلاگ **باید** `licenses-<n>` باشد، نه `license-<n>`.
            | ویوِ محصول مقصدِ خرید را با `"{$slug}-".($i+1)` می‌سازد و
            | `$slug` همان کلیدِ کاتالوگ است (`licenses`). یک حرفِ «s» کم،
            | یعنی دکمهٔ خرید پکیج را پیدا نمی‌کند و بی‌صدا به WHMCSِ بیرونی
            | برمی‌گردد — محصول در دیتابیس هست و باز هم فروخته نمی‌شود.
            */
            $slug = 'licenses-'.($i + 1);
