...
class RemindCalendar extends Command
{
    ...
    public function handle(CalendarService $calendar, AdminNotifier $admin): int
    {
         ...
         */
        /*
         * ⚠️ خطِ خالیِ ابتدا **داخلِ** همین رشته است، چون `AdminNotifier` تیتر
         * و ردیف‌ها را با یک `\n` می‌چسباند و بی‌آن سرگروهِ اول به تیتر
         * می‌چسبید.
         *
         * ⚠️ لینک عمداً `null` است (تصمیمِ کارفرما). پیام روی گوشی خوانده
         * می‌شود و آدرسِ پنل چیزی نیست که آن لحظه لازم باشد؛ یک خطِ اضافه در
         * پیامی که باید در یک نگاه خوانده شود. به همین دلیل خطِ خالیِ **انتها**
         * هم برداشته شد — بی‌لینک، فقط یک فاصلهٔ بی‌دلیل در ته پیام می‌مانْد.
         */
        $admin->event(
