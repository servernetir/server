    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $service = DB::transaction(function () use ($customer, $data, $plan, $ref, $status, $start, $request) {
            $service = Service::create([
                ...
                /*
                | 🔴 مالیات صفرِ سخت‌کد نبود که باید باشد.
                |
                | فاکتورِ تمدیدِ سرویسی که با این فرم ساخته می‌شد، مبلغ را یکجا
                | نشان می‌داد بی‌هیچ ردیفِ ارزش افزوده — در حالی که فاکتورهای
                | مسیرِ عادی تفکیک دارند. برای مشتری جمع یکی است، ولی گزارشِ
                | مالیاتیِ ما ناقص می‌شد و دو فاکتورِ هم‌مبلغِ همان مشتری دو شکل
                | داشتند (INV-…-0421 با تفکیک، INV-…-0022 بدونِ آن).
                |
                | حالا نرخ از همان جایی می‌آید که کلِ فروشگاه می‌آید، و `price`
                | مبلغِ **پیش از مالیات** است — پس فرمِ ورودی هم همان را می‌خواهد.
                */
                'tax_percent'      => \App\Http\Controllers\Account\CloudStoreController::taxPercent(),
                'cycle'            => $data['cycle'],
