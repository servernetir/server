...
class DomainController extends Controller
{
    ...
    public function retry(Domain $domain): RedirectResponse
    {
        ...
        /*
        | 🔴 بدونِ فاکتورِ پرداخت‌شده، ثبت نکن.
        |
        | ثبت یعنی خریدِ واقعی از رجیسترار. ردیفِ سفارشِ رهاشده
        | (`provision_status='none'`) نه «مرده» است نه «ثبت‌شده»، پس از هر دو
        | نگهبانِ بالا رد می‌شد — و فیلترِ پیش‌فرضِ همین صفحه دقیقاً همان‌ها را
        | نشان می‌دهد. یعنی مدیری که صفِ «نیازمندِ توجه» را با «تلاش دوباره»
        | خالی می‌کرد، ناخواسته دامنه‌های پرداخت‌نشده را می‌خرید.
        */
        if (! $domain->hasPaidInvoice()) {
            return back()->withErrors(
                'فاکتورِ این دامنه پرداخت نشده است. ثبتِ دامنه یعنی خریدِ واقعی — اول فاکتور را تسویه کنید.'
            );
        }
