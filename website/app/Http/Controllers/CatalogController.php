...
class CatalogController extends Controller
{
    ...
    private function buildLivePlans(string $iso): array
    {
        ...
        */
        ...
        |
        | در کشورهای خارجی هم‌مشخصات‌ها **یک ردیف** می‌شوند و ارزان‌ترین شهر
        | نماینده است. ایران به درخواست کارفرما استثناست: هر مشخصات × شهر ردیف
        | مستقل دارد تا ظرفیت هیچ شهر ایرانی در صفحهٔ بازاریابی پنهان نشود.
        ...
        */
        $groups = [];
        $ord = 0;
        $showEveryCityPlan = strtoupper($iso) === 'IR';

        foreach ($sellableByCity as $code => $rows) {
            foreach ($rows as $p) {
                /*
                | ایران استثناست: ظرفیت هر شهر باید به‌صورت یک ردیف مستقل دیده
                | شود. افزودن کد مکان به کلید مانع می‌شود پلن ارومیه صرفاً چون
                | همان مشخصات در تهران ارزان‌تر است، از صفحه ناپدید شود.
                |
                | درون هر شهر همچنان shelf() عرضه‌های هم‌اسلاگ را به ارزان‌ترین
                | زیرساخت سفیدبرچسب تبدیل می‌کند؛ پس ردیف خامِ تکراری نمی‌سازیم.
                */
                $k = ($showEveryCityPlan ? $code.'|' : '').self::specKey($p);
        ...
        */
        ...
        /*
        | 🔴 حذفِ پلن‌های مغلوب در کشورهای خارجی — قاعدهٔ «چیزی که هیچ‌کس نباید
        | بخرد را نشان نده». ایران عمداً از این قاعده مستثناست و تمام پلن‌های
        | قابل‌فروش آن نمایش داده می‌شوند.
        |
        ...
        */
        ...
        foreach ($groups as $g) {
            ...
            /*
            | 🔴 هر ردیف دقیقاً **یک** شهر دارد. در کشورهای خارجی آن شهر
            | ارزان‌ترینِ گروه است؛ در ایران خودِ شهر بخشی از کلید گروه است و
            | بنابراین همهٔ شهرهای قابل‌فروش ردیف مستقل دارند.
            |
            ...
            |
            | ⚠️ این قاعده برای ایران اعمال نمی‌شود؛ صفحهٔ ایران موجودی هر شهر را
            | مستقل تبلیغ می‌کند. صفحات خارجی همچنان فقط ارزان‌ترین شهر هر گروه
            | را نشان می‌دهند.
            |
            ...
            */
            ...
            $plans[] = [
                ...
                'row' => [
                    ...
                    // شهرِ خودِ ردیف؛ برای خارج نمایندهٔ ارزان‌ترین گروه و برای
                    // ایران همان شهر مستقلی است که در کلید گروه آمده است.
