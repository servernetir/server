...
/**
 * فهرستِ کشورها — ISO 3166-1 alpha-2.
 *
 * ═══ چرا (۶ شهریور ۱۴۰۵) ═══
 *
 * ستونِ customer_profiles.country «char(2)» است، ولی فرمِ KYCِ خارجی متنِ آزاد
 * («Türkiye») می‌گرفت — روی SQLiteِ محلی می‌نشست و روی MariaDBِ لایو (strict)
 * همان INSERT می‌شکست. حالا فرم انتخابگرِ کشور دارد و فقط **کد** ذخیره می‌شود؛
 * نامِ نمایشی از همین‌جا می‌آید. ردیف‌های قدیمیِ متنی هم با fallbackِ name()
 * خوانا می‌مانند.
 *
 * نام‌ها انگلیسی‌اند: مخاطبِ این فرم مشتریِ خارجی است و صفحهٔ ادمین هم با
 * همان نامِ لاتین مدرک را تطبیق می‌دهد.
 */
class Countries
{
    public const ALL = [
    ...
    /** نامِ نمایشی؛ مقدارِ ناشناخته (ردیفِ متنیِ قدیمی) عیناً برمی‌گردد. */
    ...
    /** برای <select> — مرتب بر اساسِ نام، نه کد. */
