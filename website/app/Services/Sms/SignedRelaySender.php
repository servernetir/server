...
abstract class SignedRelaySender implements SmsSender, SupportsPatterns
{
    ...
    /**
     * الگوهایی که به **کدِ یک‌بارمصرف** تعلق دارند، نه به کاتالوگِ رویدادها.
     *
     * ═══ 🔴 چرا این فهرست جداست ═══
     *
     * بقیهٔ نام‌های `TEMPLATES` رویدادِ اطلاع‌رسانی‌اند و از `NotifyEvent::ALL`
     * می‌آیند؛ `SmsTemplateRegistryTest` هم همین را قفل کرده تا الگویی برای
     * رویدادِ ناموجود ساخته نشود. ولی کدِ یک‌بارمصرف رویدادِ اطلاع‌رسانی **نیست**:
     * مسیرش `OtpService` است، به مدیر نمی‌رود، متنِ قابلِ ویرایش در
     * `/admin/templates` ندارد، و اگر در آن کاتالوگ ثبتش کنیم صفحهٔ الگوها به
     * مدیر وعدهٔ متنی می‌دهد که هیچ‌جا خوانده نمی‌شود — همان «پنلی که دروغ
     * می‌گوید» که آن فایل‌ها برای جلوگیری‌اش نوشته شدند.
     *
     * ⚠️ پس در عوض، تست می‌سنجد هر نامِ این‌جا واقعاً **هدفی** در
     * `OtpService::SMS_TEMPLATES` دارد (یا همان `otp` پیش‌فرض است). بی‌آن قید،
     * این فهرست به دری برای الگوهای مرده تبدیل می‌شد.
     *
     * ⚠️ هر عضوِ این خانواده فقط و فقط متغیرِ `code` می‌گیرد — چون هر دو
     * فرستنده (`sendOtp` و مسیرِ الگوی `OtpService`) فقط همان را می‌فرستند.
     * الگویی که در پنلِ اپراتور متغیرِ دومی داشته باشد، با `missing_param` رد
     * می‌شود و پیامکش **بی‌صدا** نمی‌رود.
     */
    public const OTP_TEMPLATES = [
        'otp',                  // کدِ ورود/ثبت‌نام/تغییرِ رمز
        'otp_service_delete',   // کدِ حذفِ کاملِ سرور — هدفِ `service_terminate`
    ];
