...
interface CloudProvider
{
    ...
    /**
     * فهرستِ **همهٔ** سرورهایی که نزدِ این زیرساخت داریم.
     *
     * دو کارِ متفاوت را ممکن می‌کند و هر دو پول‌محورند:
     *
     * ۱. **اتصالِ سرورِ دستی** — مدیر به‌جای اینکه شناسه را از پنلِ زیرساخت پیدا
     *    و دستی تایپ کند (که غلطِ تایپی و سرویسِ بی‌سرور می‌سازد)، از فهرست
     *    انتخاب می‌کند.
     * ۲. **کشفِ سرورِ یتیم** — سروری که پولش را ما به زیرساخت می‌دهیم ولی به هیچ
     *    مشتری‌ای وصل نیست. بی‌این گزارش، یک سرورِ فراموش‌شده ماه‌ها بی‌آنکه کسی
     *    بفهمد از جیبِ ما خرج می‌کند؛ دقیقاً همان ضرری که «خاتمه = حذفِ واقعی»
     *    جلویش را می‌گیرد، منتها برای سرورهایی که هرگز وارد سامانه نشدند.
     *
     * پیاده‌سازی باید صفحه‌بندیِ زیرساخت را تا ته دنبال کند؛ فهرستِ ناقص یعنی
     * سرورِ یتیمِ نادیده، که از نبودِ گزارش بدتر است چون امنیتِ کاذب می‌دهد.
     *
     * @return array{ok:bool,message:string,servers:array<int,array{ref:string,name:string,status:string,ipv4:?string,ipv6:?string,plan:?string,location:?string,created:?string}>}
     */
