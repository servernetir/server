        ...
        foreach ($this->targets($providers) as $slug) {
            ...
            foreach ((array) ($res['servers'] ?? []) as $srv) {
                ...
                $attached[] = $row + [
                    ...
                    /*
                    | 🔴 ماشینی که **هنوز نزدِ زیرساخت هست** ولی سرویسش بسته
                    | شده، «سالم» نیست — دقیقاً برعکسِ شبح است و بدترین نوعِ
                    | نشتیِ پول: اجاره‌اش می‌رود و هیچ مشتری‌ای پشتش نیست.
                    |
                    | تا امروز در همان سطلِ «وصل و سالم» می‌افتاد و صفحه با
                    | خیالِ راحت می‌گفت همه‌چیز مرتب است. شبح‌ها عمداً سرویسِ
                    | مرده را کنار می‌گذارند (نبودنِ سرورِ یک سرویسِ بسته‌شده
                    | طبیعی است)، پس **هیچ سطلی** این حالت را نمی‌گرفت.
                    */
                    'service_status' => $ci->service?->status,
                    'service_dead'   => $ci->service !== null && $ci->service->isDead(),
                    'instance_id'    => $ci->id,
