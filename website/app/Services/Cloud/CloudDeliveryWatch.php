...
class CloudDeliveryWatch
{
    ...
    {
        /*
        | 🔴 «پول‌داده» را واقعاً بسنج، نه فقط «نمرده».
        |
        | این چک ادعا می‌کند «سرویسِ ابری **پول گرفته** و تحویل نشده». ولی تنها
        | فیلترش `DEAD_STATUSES` بود — و `pending` (پیش‌فاکتورِ پرداخت‌نشده) نه
        | مرده است نه پرداخت‌شده. نتیجه روی prod: سرویسِ #۱۰۰ با فاکتورِ بازِ
        | ۱٬۸۰۴٬۰۰۰ تومان و «مجموع پرداخت‌شده: ۰» ساعت‌به‌ساعت به‌عنوانِ
        | «پول گرفته و تحویل نشده» ثبت می‌شد.
        |
        | ⚠️ ضررش فقط سروصدا نیست: اعلان **فقط روی تغییرِ وضعیت** می‌رود، پس
        | چکی که برای یک سفارشِ پرداخت‌نشده تا ابد قرمز مانده، برای تحویلِ
        | شکست‌خوردهٔ **بعدی** هیچ اعلانی نمی‌فرستد. ناظرِ همیشه‌قرمز از ناظرِ
        | نبوده بدتر است.
        |
        | `ACTIVE_STATUSES` همان تعریفِ یک‌جای «پولش داده شده» است (Service.php).
        */
        if (! in_array($service->status, Service::ACTIVE_STATUSES, true)) {
            return null;
        }
