...
class HetznerClient implements CloudProvider
{
    ...
    public function fetchCatalog(): array
    {
        ...
        /*
        | 🔴 «عرضه می‌شود؟» و «الان موجود است؟» **دو سؤالِ جدا** هستند، و تا
        | شهریور ۱۴۰۵ فقط دومی پرسیده می‌شد.
        |
        | رخداد: مشتری (سرویس #۱۳۵) در چند کشورِ مختلف سرور خرید و هر بار
        | تحویل با `[invalid_input] unsupported location for server type` شکست
        | خورد. علت این نبود که ظرفیت تمام شده باشد — آن ترکیبِ نوع/مکان **اصلاً
        | وجود ندارد**، و ما ردیفش را از جدولِ `prices` ساخته بودیم.
        |
        | ⚠️ `prices[]` یک **جدولِ تعرفه** است، نه فهرستِ مکان‌های پشتیبانی‌شده.
        | ساختنِ ردیف از رویش یعنی حاصل‌ضربِ دکارتیِ «هر نوع × هر مکان».
        |
        | پس دو نقشهٔ جدا ساخته می‌شود، و ردیفی که «عرضه نمی‌شود» باشد **اصلاً
        | ساخته نمی‌شود** — نه اینکه `in_stock=false` بگیرد. «وجود ندارد» با
        | «الان تمام شده» یکی نیست: دومی فردا برمی‌گردد، اولی هرگز.
        */
        $supported = [];
        $availability = [];
        ...
        foreach ($this->paged('/datacenters', 'datacenters') as $dc) {
            ...
            foreach ((array) data_get($dc, 'server_types.supported', []) as $id) {
                $supported[$loc][(int) $id] = true;
            }

            foreach ((array) data_get($dc, 'server_types.available', []) as $id) {
                $availability[$loc][(int) $id] = true;
                // یک نوعِ «موجود» قطعاً «عرضه‌شده» هم هست. این خط نصبی را
                // پوشش می‌دهد که `supported` را ندهد.
                $supported[$loc][(int) $id] = true;
            }
        }
        ...
        |
        | هتزنر در ۱ آوریل ۲۰۲۶ فیلدهای `datacenter.server_types.*` را منسوخ
        | اعلام کرد و گفته از ۱ اکتبر ۲۰۲۶ حذف می‌شوند. روزی که حذف شوند این
        | حلقه آرایهٔ خالی می‌سازد و — با کدِ قبلی — **همهٔ** پلن‌های این
        | زیرساخت `in_stock=false` می‌گرفتند و کلِ خط از فروشگاه غیب می‌شد،
        | با کدِ ۲۰۰ و بی‌هیچ خطایی.
        |
        | شکستِ صریح درست است: `cloud:sync` با خطا برمی‌گردد، کاتالوگِ دیروز
        ...
        */
        ...
        foreach ($this->paged('/server_types', 'server_types') as $t) {
            ...
            /*
            | فیلدِ `locations` از سپتامبر ۲۰۲۵ روی `/server_types` هست و
            ...
            */
            ...
            foreach ((array) ($t['prices'] ?? []) as $p) {
                ...
                /*
                | 🔴 گاردِ اصلیِ این رفع: ترکیبی که عرضه نمی‌شود **ردیف نمی‌گیرد**.
                |
                | تعرفه داشتن دلیلِ عرضه‌شدن نیست. بی‌این شرط، `CV-4-8` در مکانی
