        ...
        */
        ...
        $r = $this->req('POST', '/'.self::ORDERS_PATH, [
            ...
            'orders' => [[
                ...
                'name'        => $spec['name'],
                /*
                | 🔴 چرخهٔ خرید باید با چرخهٔ فروش یکی باشد.
                |
                | تا امروز این‌جا سخت‌کدِ `month` بود. یعنی مشتری **یک ساعت**
                | پول می‌داد و ما **یک ماه** از زیرساخت می‌خریدیم؛ و چون
                | `autoProlong: false` است، بعد از یک ساعت سرور حذف می‌شد و
                | آن ماهِ پرداخت‌شده می‌سوخت. روی هر فروشِ ساعتی، ضرر.
                |
                | ⚠️ هیچ خطایی تولید نمی‌کرد و در هیچ گزارشی دیده نمی‌شد —
                | فقط در صورت‌حسابِ ماهانهٔ زیرساخت. کارفرما خودش با مقایسهٔ
                | پنلِ Aeza و سفارشِ ساعتی‌اش پیدایش کرد.
                */
                'term'        => ($spec['term'] ?? 'month') === 'hour' ? 'hour' : 'month',
                /*
                |--------------------------------------------------------------
                | 🔴 ساعتی **باید** تمدیدِ خودکار داشته باشد — و ماهانه نباید
                |--------------------------------------------------------------
                |
                | رخدادِ واقعی: مشتری سرورِ ساعتی خرید و **هر ساعت** سرورش
                | می‌مرد؛ در پنلِ Aeza می‌رفت به `waiting prolongation` در حالی
                | که اعتبارِ مشتری نزدِ ما پر بود. کارفرما مجبور بود دستی تمدید
                | کند تا به `resuming` برگردد.
                |
                | علت: `autoProlong: false` را برای **همه** می‌فرستادیم، با این
                | استدلال که «تمدید را ما مدیریت می‌کنیم». ولی آن استدلال یک
                | پیش‌فرضِ نانوشته داشت: اینکه راهی برای تمدید داریم. **نداریم** —
                | SDKِ رسمیِ خودشان (`AezaGroup/aeza-net-sdk`) هیچ مسیرِ
                | prolong/renew ندارد. پس «ما مدیریت می‌کنیم» عملاً یعنی «هیچ‌کس
                | مدیریت نمی‌کند» و ماشین سرِ هر دوره می‌ایستد.
                |
                | ⚠️ چرا این ضرر نمی‌سازد: زنجیره متقارن است. تا وقتی مشتری
                | اعتبار دارد، `cloud:meter` هر ساعت از او کسر می‌کند و Aeza هم
                | هر ساعت از ما — سربه‌سر. لحظه‌ای که اعتبارش تمام شود، همان متر
                | سرویس را `suspend` و بعد `terminate` می‌کند، و `DELETE
                | services/{id}` هزینهٔ ما را قطع می‌کند. یعنی خاموش‌کردنِ تمدید
                | را **حذف** انجام می‌دهد، نه یک سوییچِ autoProlong که وجود
                | نداردش.
                |
                | ⚠️ و چرا ماهانه همچنان `false`: آن‌جا تمدید را واقعاً خودمان
                | می‌فروشیم (فاکتورِ تمدید + `services:renew-due`). اگر Aeza هم
                | خودش تمدید کند، برای ماهی که مشتری پولش را نداده هم ما پول
                | می‌دهیم و راهی برای پس‌گرفتن نیست.
                |
                | ⚠️ ریسکِ باقی‌مانده که باید بدانیم: اگر متر به هر دلیلی نایستد
                | (کرون بخوابد)، Aeza تا وقتی موجودیِ حسابِ ما هست ادامه می‌دهد.
                | `SystemHealth` چکِ کرون را دارد و همان اولین خطِ دفاع است.
                */
                'autoProlong' => ($spec['term'] ?? 'month') === 'hour',
                'parameters'  => $params,
            ]],
        ]);
