    ...
    private function persist(array $row, \Illuminate\Support\Carbon $now): void
    {
        ...
        */
        ...
        if ($isLeaking) {
            ...
            // «بی‌درآمد نیست» یعنی یا به مشتری وصل است یا اصلاً ماشینی نمانده؛
            // هر دو حالت ساعتِ رهاشدگی را بی‌موضوع می‌کنند.
            $asset->unlinked_since = null;

            // ولی تأییدِ کهنه فقط با **بازگشت به مشتری** پاک می‌شود: تصمیمی که
            // مدیر دربارهٔ نشتیِ قبلی گرفته بود، نباید نشتیِ بعدیِ همین ماشین را
            // خاموش نگه دارد. برای شبح دست نمی‌زنیم — ماشینش رفته و بازکردنِ
            // دوبارهٔ یک هشدارِ بسته‌شده هیچ سودی ندارد.
            if ($row['link_state'] === InfraAsset::STATE_ATTACHED) {
