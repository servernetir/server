...
class MailboxDigest
{
    ...
    public function send(bool $dry = false): array
    {
        ...
        try {
            ...
            | 🔴 `toAdmin()` و **نه** `notify()`.
            |
            | `notify()` خطِ اولش `BaleSafirSender` است و سفیر به‌ازای **هر
            | پیام** پول می‌گیرد. این خلاصه مالِ کارفرماست نه مشتری، پس باید از
            | APIِ رباتِ خودمان برود که رایگان است.
            |
            | ⚠️ همان قاعده‌ای که برای `AdminNotifier` نوشته شده بود و این‌جا
            | اجرا نمی‌شد: خلاصهٔ روزانهٔ صندوقِ ایمیل بی‌سروصدا از کانالِ پولی
            | می‌رفت. کارفرما خودش دید («از مسیر سفیر بله دارد می‌فرستد»).
            |
            | ⚠️ درسِ عمومی‌تر: قاعده‌ای که فقط در docblock نوشته شود ولی هیچ
            | تستی نداشته باشد، با هر فراخوانِ تازه دوباره شکسته می‌شود. حالا
            | `BaleAdminChannelTest` سورس را می‌پاید.
            */
