...
class LicenseOrderTicket
{
     ...
     * فقط سفارشِ لایسنس.
     *
     * نشانه `server_ip` است: تنها پکیجی آن را دارد که `requires_server_ip`
     * داشته و مشتری در فرمِ سفارش پرش کرده. عمداً به `category` یا نامِ پکیج
     * گره نخورده — آن‌ها متنِ بازاریابی‌اند و فردا عوض می‌شوند.
     */
    public static function applies(Service $service): bool
    {
        return filled($service->server_ip)
    ...
    private static function body(Service $service): string
    {
        return implode("\n", [
            ...
            '',
            'پکیج: '.$service->name,
            'IP سرور برای فعال‌سازی: '.$service->server_ip,
            '',
