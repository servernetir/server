...
class AdminBaleGate
{
    ...
    private const STAMP_WINDOW = 900;

    /**
     * مهرِ کوتاهی که به `callback_data` چسبانده می‌شود.
     *
     * ═══ مسئله‌ای که حل می‌کند ═══
     *
     * 🔴 دکمه‌های بله در تاریخچهٔ چت **تا ابد کلیک‌شدنی** می‌مانند. یک
     * «⏸ تعلیق» که سه هفته پیش زیرِ کارتِ یک سرویس فرستاده شده، امروز با یک
     * کلیکِ اتفاقی سرویسِ زندهٔ مشتری را می‌خواباند — و مشتری پیامکِ «سرویس شما
     * غیرفعال شد» می‌گیرد.
     *
     * ═══ چرا مهر و نه یک مرحلهٔ تأییدِ اضافه ═══
     *
     * کارفرما تأییدِ اضافه را برداشت و حق داشت: اصطکاک روی گوشی یعنی برگشتن به
     * پنل. این مهر **در جریانِ عادی نامرئی است** — تازه که باشد کار انجام
     * می‌شود، کهنه که باشد ربات می‌گوید کارت را دوباره باز کنید. هزینه‌اش صفر
     * تپ است و فقط دکمه‌های کهنه را می‌گیرد.
     *
     * ═══ چرا بی‌حالت ═══
     *
     * ⚠️ نگه‌داشتنِ nonceها در blobِ وضعیت یعنی همان blob که منتقد سرش هشدار داد
     * مدام بزرگ‌تر و پرنویس‌تر شود. مهر از **خودِ محتوا و پنجرهٔ زمانی** ساخته
     * می‌شود: هیچ‌چیز ذخیره نمی‌شود، خودبه‌خود منقضی می‌شود، و بی‌`APP_KEY`
     * قابلِ جعل نیست.
     */
    public function stamp(string $payload): string
    {
        return $this->stampFor($payload, (int) floor(now()->getTimestamp() / self::STAMP_WINDOW));
    }

    /** پنجرهٔ جاری **یا** پنجرهٔ قبلی — تا کلیکِ سرِ مرز رد نشود */
    public function verifyStamp(string $payload, string $stamp): bool
    {
        if ($stamp === '') {
            return false;
        }

        $now = (int) floor(now()->getTimestamp() / self::STAMP_WINDOW);

        foreach ([$now, $now - 1] as $window) {
            if (hash_equals($this->stampFor($payload, $window), $stamp)) {
                return true;
            }
        }
    ...
    private function stampFor(string $payload, int $window): string
    {
        return substr(hash_hmac('sha256', $payload.'|'.$window, (string) config('app.key')), 0, 4);
    }
