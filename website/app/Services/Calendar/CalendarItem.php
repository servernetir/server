...
final class CalendarItem
{
    ...
    /**
     * مبلغِ قابلِ نمایشِ این رویداد، یا نال اگر پولی در کار نیست.
     *
     * ⚠️ هر لایه مبلغ را زیرِ کلیدِ خودش می‌گذارد — `amount` (یادآوریِ دستی)،
     * `total` (فاکتور)، `price` (سرویس)، `renew_toman` (دامنه). این تفاوت
     * تاریخی و طبیعی است، ولی مصرف‌کننده نباید مجبور باشد هر چهار را بشناسد؛
     * وگرنه هر جای تازه‌ای که مبلغ می‌خواهد، یکی از آن‌ها را جا می‌اندازد و
     * آن لایه بی‌صدا بی‌مبلغ می‌مانَد.
     *
     * ترتیب مهم است: `total`ِ فاکتور پیش از `price` می‌آید چون فاکتور هر دو
     * را ندارد ولی اگر روزی داشت، مبلغِ کلِ فاکتور درست‌تر است.
     */
    public function money(): ?string
    {
        $currency = (string) ($this->meta['currency'] ?? '') ?: 'IRT';

        foreach (['amount', 'total', 'price', 'renew_toman'] as $key) {
            $value = (int) ($this->meta[$key] ?? 0);

            if ($value > 0) {
                return invoice_money($value, $currency);
            }
        }
