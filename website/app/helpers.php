...
if (! function_exists('article_faq_ld')) {
    ...
    function article_faq_ld(string $html): string
    {
        ...
        /*
         * 🔴 «sıkça» تنها شکلِ ترکیِ این عنوان نیست — و رایج‌ترینش هم نیست.
         *
         * مترجم روی مقالهٔ واقعی «Sık Sorulan Sorular» نوشت (همان که SSS
         * خلاصه‌اش است) و الگو فقط «sıkça sorulan sorular» داشت. نتیجه:
         * **هیچ مقالهٔ ترکی‌ای اسکیمای FAQ نگرفت** — نه خطایی، نه لاگی، فقط
         * نبودِ یک تگ در صفحه‌ای که ۲۰۰ می‌دهد. فارسی و انگلیسی سالم بودند،
         * پس نمونه‌گیریِ یک‌زبانه هم پیدایش نمی‌کرد.
         *
         * درس: وقتی متن را یک مدل تولید می‌کند، عنوان **یک رشتهٔ ثابت نیست**؛
         * الگو باید گونه‌های طبیعیِ همان عبارت را بپذیرد، نه دقیقاً آنچه
         * اولین بار دیدی. تستِ `TrilingualFaqSchemaTest` هر سه زبان را با
         * چند گونه می‌سنجد.
         */
        $heads = 'پرسش(?:‌| )?های (?:پرتکرار|متداول|رایج)|سوالات (?:متداول|رایج)'
            .'|frequently asked questions|common questions|faq'
            .'|sık(?:ça)? sorulan sorular|sss';
