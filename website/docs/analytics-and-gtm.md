...
## قرارداد اجرایی نسخهٔ سوم — اندازه‌گیری همیشه‌فعال، درآمد و تصمیم رشد

- بنا بر تصمیم مالک محصول، GTM و GA4 بدون نمایش پنجرهٔ رضایت برای همهٔ
  بازدیدکنندگان فعال‌اند و `analytics_storage` پیش‌فرض `granted` است.
- همهٔ مجوزهای تبلیغاتی (`ad_storage`، `ad_user_data` و
  `ad_personalization`) همچنان `denied` می‌مانند. شناسهٔ مشتری، دامنهٔ کامل،
  IP و مقدار کپی‌شده ارسال نمی‌شود.
