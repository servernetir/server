    ...
    private function catalog(): array
    {
        return [
            ...
            */
            ...
            // ─────────────── نمایندگی دامنه ───────────────
            [
                'key' => 'reseller_level_changed', 'title' => 'تغییر سطح نمایندگی دامنه', 'group' => 'other',
                /*
                | ⚠️ `sms_event` عمداً نال است.
                |
                | متنِ پیامک نزدِ اپراتور است و ما فقط کدِ الگو و متغیرها را
                | می‌فرستیم. برای رویدادی که هنوز الگویی در پنلِ اپراتور ندارد،
                | گذاشتنِ یک کدِ ساختگی یعنی `SmsDispatcher::event()` الگو را
                | «پیدا» می‌کند، ارسال شکست می‌خورد، و **متنِ آزاد را جایگزین
                | نمی‌فرستد** — پس پیام بی‌صدا گم می‌شود. نال یعنی ایمیل و بله
                | کارشان را می‌کنند.
                */
                'sms_event' => null,
                'email_subject' => 'سطح نمایندگی شما تغییر کرد',
                'email_body' => '<p>سطح نمایندگی دامنهٔ شما اکنون <b>{level}</b> است.</p>'
                    .'<p>تخفیف مؤثر شما: <b>{discount}٪</b> روی قیمت خرده‌فروشی.</p>'
                    .'<p>جزئیات و پیشرفت تا سطح بعد در پنل نمایندگی شماست.</p>',
                'bale_body' => 'سطح نمایندگی دامنهٔ شما اکنون «{level}» است — تخفیف {discount}٪.',
                /*
                | ⚠️ این فهرست باید **دقیقاً** همان چیزی باشد که
                | `ReviewResellerTiers` می‌فرستد (`level` و `discount`).
                | چیپِ متغیری که کد نمی‌فرستد، در «ارسال آزمایشی» درست دیده
                | می‌شود ولی سرِ ارسالِ واقعی جای‌نگهدارش پر نمی‌ماند و محافظِ
                | `incomplete()` کلِ الگو را کنار می‌گذارد — یعنی ویرایشِ مدیر
                | برای همیشه بی‌اثر می‌شود، بی‌هیچ خطایی.
                */
                'variables' => [
                    ['name' => 'level', 'desc' => 'نام سطح جدید'],
                    ['name' => 'discount', 'desc' => 'درصد تخفیف مؤثر'],
                ],
            ],

            /*
            | رسیدِ واریزِ بانکی — تا شهریور ۱۴۰۵ مخاطبش فقط مدیر بود، پس
            | ردیفی هم لازم نداشت. حالا که کارفرما الگوی پیامکِ خطاب‌به‌مشتری
            | برایش ساخت و مخاطب `BOTH` شد، بی‌این ردیف پیام به مشتری می‌رفت
            | و مدیر در `/admin/templates` اثری از آن نمی‌دید — همان «پنلی که
            | دروغ می‌گوید» که `NotifyPanelTruthTest` برای گرفتنش نوشته شده.
            */
            [
                'key' => 'bank_receipt', 'title' => 'رسیدِ واریز، در انتظارِ تأیید', 'group' => 'billing',
                'sms_event' => 'bank_receipt',
                'email_subject' => 'رسید واریز شما برای فاکتور {number} ثبت شد',
                'email_body' => '<p>رسید واریز شما به مبلغ <b>{amount}</b> برای پیش‌فاکتور <b>{number}</b> دریافت شد.</p>'
                    .'<p>پس از بررسی، نتیجه را همین‌جا اطلاع می‌دهیم.</p>',
                'bale_body' => 'رسید واریز شما به مبلغ {amount} (فاکتور {number}) ثبت شد و در انتظار بررسی است.',
                'variables' => [
                    ['name' => 'number', 'desc' => 'شمارهٔ پیش‌فاکتور'],
                    ['name' => 'amount', 'desc' => 'مبلغ واریزی'],
                ],
            ],

            // ─────────────── اعتبارِ سرویسِ ساعتی ───────────────
            /*
            | 🔴 این دو تا شهریور ۱۴۰۵ اصلاً الگو نداشتند و فقط متنِ سخت‌کدِ
            | `CloudMeterHourly` می‌رفت. یعنی فوری‌ترین پیامِ مالیِ سامانه
            | ویرایش‌پذیر نبود و پیامکش هم نمی‌رفت.
            |
            | ⚠️ `variables` باید دقیقاً همان چیزی باشد که
            | `CloudMeterHourly::notifyCustomer` پاس می‌دهد (`service` را خودش
            | اضافه می‌کند). چیپِ اضافه ⇒ `incomplete()` کلِ الگو را کنار
            | می‌گذارد و ویرایشِ مدیر بی‌اثر می‌شود.
            */
            [
                'key' => 'hourly_low_credit', 'title' => 'اعتبارِ سرویسِ ساعتی رو به اتمام', 'group' => 'billing',
                'sms_event' => 'hourly_low_credit',
                'email_subject' => 'اعتبار سرویس {service} رو به اتمام است',
                'email_body' => '<p>اعتبار کیف پول شما فقط برای حدود <b>{hours} ساعت</b> دیگرِ سرویس «{service}» کافی است.</p>'
                    .'<p>برای جلوگیری از خاموش شدن سرور، کیف پول را شارژ کنید.</p>',
                'bale_body' => 'اعتبار سرویس «{service}» فقط برای {hours} ساعت دیگر کافی است. کیف پول را شارژ کنید.',
                'variables' => [
                    ['name' => 'service', 'desc' => 'نام سرویس'],
                    ['name' => 'hours', 'desc' => 'ساعت باقی‌مانده'],
                ],
            ],
            [
                'key' => 'hourly_credit_out', 'title' => 'اتمامِ اعتبار و خاموشیِ سرویسِ ساعتی', 'group' => 'billing',
                'sms_event' => 'hourly_credit_out',
                'email_subject' => 'سرویس {service} به دلیل اتمام اعتبار خاموش شد',
                'email_body' => '<p>اعتبار سرویس «{service}» تمام شد و سرور موقتاً خاموش شد.</p>'
                    .'<p>با شارژ کیف پول، سرور خودکار روشن می‌شود؛ در غیر این صورت پس از <b>{grace} ساعت</b> حذف خواهد شد.</p>',
                'bale_body' => 'اعتبار سرویس «{service}» تمام شد و سرور خاموش شد. پس از {grace} ساعت حذف می‌شود.',
                'variables' => [
                    ['name' => 'service', 'desc' => 'نام سرویس'],
                    ['name' => 'grace', 'desc' => 'ساعت مهلت تا حذف'],
                ],
            ],

            // ─────────────── تولد ───────────────
            [
                'key' => 'birthday', 'title' => 'تبریکِ تولد و اعتبارِ هدیه', 'group' => 'other',
                'sms_event' => 'birthday',
                'email_subject' => 'تولدتان مبارک، {name} عزیز',
                'email_body' => '<p>{name} عزیز، تولدتان مبارک!</p>'
                    .'<p>به همین مناسبت یک کد هدیه <b>{credit} تومانی</b> برای شما صادر شد:</p>'
                    .'<p style="font-size:20px;letter-spacing:2px"><b>{code}</b></p>'
                    .'<p>این کد فقط برای حساب خودتان کار می‌کند و تا <b>{hours} ساعت</b> اعتبار دارد؛'
                    .' هنگام پرداخت فاکتور واردش کنید.</p>',
                'bale_body' => '{name} عزیز، تولدتان مبارک! کد هدیه {credit} تومانی شما: {code} — تا {hours} ساعت معتبر.',
                'variables' => [
                    ['name' => 'name', 'desc' => 'نام کوچک مشتری'],
                    ['name' => 'credit', 'desc' => 'مبلغ هدیه به تومان'],
                    ['name' => 'code', 'desc' => 'کد هدیه'],
                    ['name' => 'hours', 'desc' => 'مدت اعتبار (ساعت)'],
                ],
            ],
        ];
    }
