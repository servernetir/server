    ...
    private function catalog(): array
    {
        return [
            ...
            */
            ...
            /*
            | ⚠️ `sms_event` عمداً خالی است — و این‌جا دلیلش از «الگو نساخته‌ایم»
            | قوی‌تر است: این رویداد از `CustomerNotifier::templated()` می‌رود که
            | **اصلاً مسیرِ پیامک ندارد**. پس مسئله «الگو یادمان رفت» نیست.
            |
            | و انتخابِ `templated()` هم آگاهانه بود: انقضای یک پیش‌فاکتورِ
            | پرداخت‌نشده خبرِ فوری نیست و پیامکِ پولی نمی‌ارزد؛ ولی چون ما خودمان
            | لحظهٔ سفارش ایمیلی با **لینکِ زندهٔ همان فاکتور** فرستاده‌ایم، انقضای
            | بی‌صدا آن ایمیل را به قولی شکسته تبدیل می‌کند.
            */
            [
                'key' => 'invoice_expired', 'title' => 'انقضای پیش‌فاکتورِ پرداخت‌نشده', 'group' => 'billing',
                'sms_event' => null,
                'email_subject' => 'پیش‌فاکتور {number} منقضی شد',
                'email_body' => '<p>پیش‌فاکتور شماره <b>{number}</b> پرداخت نشد و منقضی شد.</p>'
                    .'<p>چیزی از دست نرفته است: قیمت‌ها نوسان دارند، پس کافی است سفارشِ تازه با قیمتِ روز ثبت کنید.</p>',
                'bale_body' => 'پیش‌فاکتور {number} پرداخت نشد و منقضی شد. برای خرید، سفارشِ تازه با قیمتِ روز ثبت کنید.',
                'variables' => [
                    ['name' => 'number', 'desc' => 'شمارهٔ پیش‌فاکتور'],
                ],
            ],
            [
            ...
            /*
            | ⚠️ `sms_event` عمداً خالی است: برای این کلید الگویی در پنلِ اپراتور
            | ساخته نشده، پس پیامک نمی‌رود و فقط ایمیل و بله می‌ماند.
            | `SmsTemplateRegistryTest` همین تصمیم را صریح قفل کرده.
            */
            [
                'key' => 'service_hold', 'title' => 'نگه‌داشتنِ سفارش برای بازبینی', 'group' => 'service',
                'sms_event' => null,
                'email_subject' => 'سفارش {service} در حالِ بررسی است',
                'email_body' => '<p>سفارش <b>{service}</b> ثبت و پرداخت شد، ولی پیش از تحویل یک بررسیِ کوتاهِ امنیتی روی آن انجام می‌شود.</p>'
                    .'<p>این کار برای همهٔ سفارش‌ها روتین است و معمولاً در کمتر از چند ساعتِ کاری تمام می‌شود. '
                    .'به‌محضِ تأیید، سرور به‌صورت خودکار ساخته می‌شود و مشخصاتِ ورود را همین‌جا و با ایمیل دریافت می‌کنید.</p>'
                    .'<p>مبلغی از دست نمی‌رود. اگر بررسی به نتیجه نرسد، وجه به‌طور کامل به کیفِ پولِ شما برمی‌گردد.</p>',
                'bale_body' => 'سفارش {service} پیش از تحویل در حالِ بررسیِ کوتاه است. نتیجه را به شما اطلاع می‌دهیم؛ مبلغی از دست نمی‌رود.',
                'variables' => [
                    ['name' => 'service', 'desc' => 'نام سرویس'],
                ],
            ],
