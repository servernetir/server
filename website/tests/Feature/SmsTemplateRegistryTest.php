...
class SmsTemplateRegistryTest extends TestCase
{
    ...
    public function test_the_events_without_an_sms_pattern_are_explicitly_known(): void
    {
        $expected = [
            ...
            /*
            | پاسخِ تازهٔ مشتری در تیکت — مخاطبش **فقط مدیر** است
            | (`NotifyEvent::ADMIN`). `Notifier::fire()` برای رویدادِ
            | ادمین‌محور اصلاً سراغِ `CustomerNotifier` نمی‌رود، پس هیچ
            | پیامکی در کار نیست که الگو بخواهد — نه «الگو یادمان رفت»،
            | بلکه «مسیرش وجود ندارد».
            |
            | ⚠️ اگر روزی مخاطبش به `BOTH` تغییر کرد، این تست قرمز **نمی‌شود**
            | ولی مشتری بی‌پیامک می‌مانَد. مخاطب‌ها را `NotificationCoverageTest`
            | جدا قفل کرده؛ آن‌جا را هم نگاه کن.
            */
            'ticket_customer_reply',
        ];

        /*
        | ⚠️ سه ردیف در ۱۵ شهریور ۱۴۰۵ از این فهرست بیرون رفتند —
        | `service_hold`، `bank_receipt`، `invoice_expired` — چون کارفرما
        | الگوهایشان را در آی‌پی‌پنل ساخت و تأیید گرفت.
        |
        | 🔴 و یک ادعای **کهنه** هم با آنها رفت: کامنتِ قبلیِ `invoice_expired`
        | می‌گفت «افزودن به این دو فهرست کافی نیست، فراخوان هم باید از
        | `templated()` به `Notifier::fire()` عوض شود». آن جمله از دورانی
        | مانده بود که `templated()` کلیدِ `'__none__'` پاس می‌داد. امروز
        | همان متد کلیدِ **واقعی** را به `SmsDispatcher::event()` می‌دهد
        | (خودِ `CustomerNotifier::templated` مفصل توضیحش داده)، پس افزودن به
        | دو فهرست *کافی است*.
        |
        | درسش عمومی‌تر است: کامنتی که می‌گوید «برای کارِ X باید Y را هم عوض
        | کنی» با هیچ تستی قفل نمی‌شود، پس وقتی Y عوض شود کامنت بی‌صدا دروغ
        | می‌شود — و نفرِ بعدی یا کارِ اضافه می‌کند یا کارِ اشتباه.
        */
        sort($expected);
