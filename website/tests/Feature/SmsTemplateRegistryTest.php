    ...
    public function test_the_events_without_an_sms_pattern_are_explicitly_known(): void
    {
        $expected = [
            ...
            */
            ...
            /*
            | انقضای پیش‌فاکتورِ پرداخت‌نشده — **تصمیمِ آگاهانه**: پیامک نه.
            |
            | برخلافِ `service_hold` که «الگویش را در اپراتور نساخته‌ایم»، این‌جا
            | مسیرِ پیامک اصلاً وجود ندارد: `ExpireOrderInvoices` از
            | `CustomerNotifier::templated()` می‌رود که عمداً بی‌پیامک است. پس
            | هیچ تلاشِ ناموفقی هم ثبت نمی‌شود — برخلافِ `service_hold` که سرِ هر
            | بار یک ردیف در ردیابِ خطا می‌گذارد.
            |
            | خودِ تصمیم: انقضای یک پیش‌فاکتورِ پرداخت‌نشده خبرِ فوری نیست و پیامکِ
            | پولی نمی‌ارزد؛ ایمیل و بله کافی‌اند.
            |
            | ⚠️ اگر روزی خواستی پیامکش برود، افزودن به این دو فهرست **کافی
            | نیست** — فراخوان هم باید از `templated()` به `Notifier::fire()`
            | عوض شود، وگرنه الگو ساخته می‌شود و هیچ پیامکی درخواست نمی‌شود.
            */
            'invoice_expired',
        ];
        sort($expected);
