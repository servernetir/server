...
class AdminTicketSortingTest extends TestCase
{
    ...
    {
        return User::create(['name' => 'A', 'email' => 'a@sort.test', 'password' => bcrypt('x'), 'role' => 'admin']);
    }

    private function customer(): Customer
    {
        return Customer::create(['code' => 'SN-SORT', 'email' => 'c@sort.test', 'phone' => '09120000111', 'password' => bcrypt('x'), 'status' => 'active', 'locale' => 'fa']);
    }
    ...
    private function ticket(Customer $c, string $subject, string $status, string $created, string $reply): Ticket
    {
        ...
        $t->forceFill(['created_at' => Carbon::parse($created), 'updated_at' => Carbon::parse($created)])->saveQuietly();
    ...
    {
        $c = $this->customer();
        $a = $this->ticket($c, 'a', 'answered', '2026-01-01', '2026-06-01');
        $b = $this->ticket($c, 'b', 'answered', '2026-01-02', '2026-05-01');
        Carbon::setTestNow('2026-07-01');
        $b->addMessage('customer', $c->id, null, 'customer');
        Carbon::setTestNow('2026-08-01');
        $a->addMessage('staff', $this->admin()->id, 'A', 'staff');
        Carbon::setTestNow();
    ...
    public function test_answered_ticket_remains_in_default_list_after_staff_reply(): void
    {
        Http::fake();
        Mail::fake();
        $customer = $this->customer();
        $admin = $this->admin();
        $answered = $this->ticket($customer, 'مشکل اصلی', 'open', '2026-01-01', '2026-05-01');

        $this->actingAs($admin)->get('/admin/tickets')
            ->assertOk()->assertSee($answered->number);

        Carbon::setTestNow('2026-06-01 10:00:00');
        $this->actingAs($admin)->post('/admin/tickets/'.$answered->id.'/reply', [
            'body' => 'پاسخ پشتیبانی',
        ])->assertRedirect('/admin/tickets');
        Carbon::setTestNow();

        $this->assertSame('answered', $answered->fresh()->status);
        // GET واقعی بلافاصله پس از پاسخ: هم regression اصلی را می‌بندد و هم
        // flash شمارهٔ تیکت را پیش از سنجش ترتیب مصرف می‌کند.
        $this->actingAs($admin)->get('/admin/tickets')
            ->assertOk()->assertSee($answered->number);

        // این تیکت هنوز نیازمند پاسخ است و باید بالاتر از answered بماند.
        $waiting = $this->ticket($customer, 'هنوز منتظر', 'open', '2026-01-02', '2026-04-01');
        $order = $this->order('');

        $this->assertContains($answered->number, $order);
        $this->assertLessThan(
            array_search($answered->number, $order, true),
            array_search($waiting->number, $order, true),
            'تیکت نیازمند پاسخ باید پیش از تیکت پاسخ‌داده‌شده باشد',
        );
    }
}
