...
class AdminCreditAdjustTest extends TestCase
{
    ...
    /**
     * 🔴 دکمه‌ای که هیچ کاری نمی‌کند — بی‌خطا، بی‌دیالوگ، بی‌لاگ.
     *
     * ═══ رخداد (شهریور ۱۴۰۵) ═══
     * فیلدِ مبلغ `min="1" step="1000"` داشت. پایهٔ گام در HTML خودِ `min` است،
     * نه صفر — پس مقادیرِ معتبر ۱، ۱۰۰۱، ۲۰۰۱ … بودند و **هر مبلغِ گردی
     * نامعتبر**. مرورگر رویدادِ submit را اصلاً شلیک نمی‌کرد: نه خطایی در
     * کنسول، نه دیالوگِ تأیید، نه ارسالی. و چون فیلد در تبِ بسته بود، حبابِ
     * اعتبارسنجی هم دیده نمی‌شد.
     *
     * ⚠️ ادعا عمداً روی **رشتهٔ `step="1"` نیست**، روی همان حسابی است که
     * مرورگر می‌کند: `(مقدار − min) % step === 0`. وگرنه هر بار که کسی min را
     * عوض کند، تست سبز می‌مانَد و باگ برمی‌گردد.
     */
    public function test_the_amount_field_actually_accepts_round_amounts(): void
    {
        $c = $this->customer();

        $html = $this->actingAs($this->admin)
            ->get('/admin/customers/'.$c->id)->getContent();

        $this->assertSame(1, preg_match(
            '/<input[^>]*name="amount"[^>]*>/',
            substr($html, strpos($html, '/credit')),
            $m,
        ), 'فیلدِ مبلغِ کیفِ پول در صفحه نیست.');

        preg_match('/min="(\d+)"/', $m[0], $min);
        preg_match('/step="(\d+)"/', $m[0], $step);

        $minV  = (int) ($min[1] ?? 0);
        $stepV = (int) ($step[1] ?? 1);

        foreach ([1000, 50_000, 594_000, 1_000_000] as $amount) {
            $this->assertSame(0, ($amount - $minV) % $stepV,
                "مبلغِ {$amount} با min={$minV} و step={$stepV} از نظرِ مرورگر نامعتبر است — "
                .'فرم بی‌صدا ارسال نمی‌شود.');
        }
    }
