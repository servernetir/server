...
class AssistantLeadCaptureTest extends TestCase
{
    ...
    {
        parent::setUp();
        config(['services.bale.token' => 'bot', 'servernet.contact.notify_phones' => [], 'servernet.contact.notify_chat_ids' => ['101', '202'], 'servernet.contact.email' => '']);
    ...
    {
        $s = app(AssistantLeadCapture::class);
        $data = ['name' => 'علی', 'phone' => '09120000000', 'email' => 'lead@example.test', 'summary' => 'سرور ایران', 'product' => 'VPS'];
        $first = $s->capture($data, 'session-1', 'fa', 'https://servernet.cloud/vps', null);
        $second = $s->capture(array_merge($data, ['summary' => 'خلاصه تازه']), 'session-1', 'fa', 'https://servernet.cloud/vps', null);
        $this->assertSame($first->id, $second->id);
        $this->assertSame(1, CrmLead::where('source', 'assistant')->count());
        $this->assertSame('خلاصه تازه', $second->fresh()->observation);
        Http::assertSentCount(2);
        foreach (['101', '202'] as $chat) {
            Http::assertSent(fn ($r) => (string) ($r->data()['chat_id'] ?? '') === $chat);
        }
    ...
    {
        Http::fake(fn () => throw new ConnectionException('bale unavailable'));
    ...
    public function test_assistant_leads_are_filterable_and_stage_change_is_audited(): void
    {
        ...
        $this->assertDatabaseHas('activity_logs',['action' => 'crm_lead_stage', 'actor' => 'staff']);
    }
}
