...
class DomainPurchaseTest extends TestCase
{
     ...
     *
     * ⚠️ مقصد عمداً **صفحهٔ تسویهٔ همان دامنه** است، نه صفحهٔ پروفایل: فرمِ
     * مالک همان‌جاست، پس مشتری پرش می‌کند و خریدش را ادامه می‌دهد. فرستادنش
     * به پروفایل یعنی از جریانِ خرید بیرون می‌افتد و اغلب برنمی‌گردد.
     *
     * ═══ چرا این تست مدتی قرمز بود، و درسِ گران‌ترش ═══
     *
     * ادعای قدیمی `route('account.profile')` بود — از طراحیِ قبلی مانده بود و
     * وقتی مقصد عوض شد، به‌روز نشد. تا این‌جا یک تستِ کهنهٔ معمولی.
     *
     * 🔴 ولی چیزی که وقت گرفت این بود که PHPUnit **پیامِ درست را نشان نمی‌داد**.
     * به‌جای «۳۰۲ به X انتظار می‌رفت، Y آمد» این را می‌داد:
     *
     *     Call to a member function all() on array
     *
     * علتش `config/session.php` → `'serialization' => 'json'` است (عمدی، برای
     * بستنِ حملهٔ gadget-chain اگر APP_KEY لو برود). با آن، هر **شیئی** در نشست
     * به آرایه تبدیل می‌شود؛ و لاراول موقعِ ساختنِ پیامِ شکستِ `assertRedirect`
     * سعی می‌کند خطاهای اعتبارسنجی را ضمیمه کند و روی همان آرایه `all()` صدا
     * می‌زند.
     *
     * ⚠️ پس **هر** ادعای شکست‌خورده روی درخواستی که خطای نشست دارد، همین پیامِ
     * گمراه‌کننده را می‌دهد. اگر روزی دوباره دیدیش: دنبالِ باگِ نشست نگرد —
     * ادعای واقعی را پیدا کن که شکسته.
     *
     * ✅ و خودِ اپ سالم است: در ویو `$errors` یک `ViewErrorBag`ِ واقعی است،
     * `any()` کار می‌کند و پیام روی صفحه دیده می‌شود. این فقط ابزارِ تست است.
     */
    public function test_a_customer_without_a_profile_is_sent_to_complete_it(): void
    {
        ...
        $this->assertSame(0, Invoice::count(), 'فاکتور ساخته شد — یعنی مسیرِ پول پیش از گیتِ مالک باز بود');
        $this->assertSame(0, Domain::count());
    }
