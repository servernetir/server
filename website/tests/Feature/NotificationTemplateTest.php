...
class NotificationTemplateTest extends TestCase
{
    ...
    /**
     * 🔴 `templated()` نباید الگوی **پیامک** را صدا بزند.
     *
     * `SmsDispatcher` عمداً وقتی الگویی پیدا شود و ارسالش شکست بخورد متنِ آزاد
     * را جایگزین نمی‌فرستد. پس اگر یادآوریِ تمدید از مسیرِ الگوی اپراتور برود و
     * متغیرهایش نخوانَد، پیام بی‌صدا گم می‌شود و مشتری بی‌خبر قطع می‌شود.
     */
    public function test_templated_never_uses_an_sms_pattern(): void
    {
        $seen = [];
        ...
        app(CustomerNotifier::class)->templated($this->customer(), 'expiring', [], 'متن قدیمی');

        $this->assertSame(['__none__'], $seen, 'کلیدِ رویداد نباید به لایهٔ پیامک برود');
    }
    // ═══════════════════ صفحهٔ مدیریت ═══════════════════
