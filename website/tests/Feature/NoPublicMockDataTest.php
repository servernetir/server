...
class NoPublicMockDataTest extends TestCase
{
     ...
     *
     * ⚠️ کامنت‌ها دو بار برگشتند (ممیزی ۷ و شهریور ۱۴۰۵)، هر بار با نیتِ خوبِ
     * «بعداً یادمان بماند چرا». پس **دلیلِ هر قاعده این‌جا می‌مانَد**، تا جای
     * درستش وجود داشته باشد و کسی وسوسه نشود در خودِ فایل بنویسد:
     *
     *   · `/api/`     — سطحِ API، نه محتوا. خزیدنش فقط سهمیه می‌سوزاند.
     *   · `/system/`  — مسیرهای موقتیِ مهاجرت و عیب‌یابی.
     *   · `/go/`      — ریدایرکتِ شمارش‌پذیرِ پرداخت است، نه محتوا (ممیزی ۷).
     *   · `/account/` — پنلِ مشتری `noindex` است ولی همچنان خزیده می‌شد و
     *     ده‌ها URLِ پارامتریِ cloud-store داشت؛ بودجهٔ خزش را می‌سوزاند
     *     (GSC، شهریور ۱۴۰۵). بستنِ خزش، بودجه را به ۶۵۰+ صفحهٔ
     *     «Discovered - not indexed» برمی‌گرداند.
     *   · `/login`    — به همان دلیل؛ صفحهٔ ورود محتوای قابلِ ایندکس نیست.
     */
