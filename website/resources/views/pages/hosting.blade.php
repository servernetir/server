...
@php
  ...
  foreach ($product['plans'] as $sdP) {
      ...
      /*
      | 🔴 نیمهٔ فراموش‌شدهٔ همان باگِ `eur`.
      |
      | `site_price()` محافظ گرفت و صفحه دیگر ۵۰۰ نمی‌دهد، ولی این‌جا `?? 0`
      | مانده بود: صفحاتِ دامنه عمداً `eur` ندارند (قیمت زنده و تومانی است)،
      | پس روی `/en` و `/tr` یک `Offer` با «price: 0, priceCurrency: EUR»
      | منتشر می‌شد — یعنی به گوگل و مدل‌های زبانی می‌گفتیم دامنه **رایگان**
      | است. همان چیزی که کامنتِ `site_price()` صریح ممنوعش کرده: «€۰ یعنی
      | رایگان، و آن از خطا بدتر است.» ۵۰۰ را کارفرما در ردیاب می‌بیند؛ این
      | یکی را هیچ‌کس نمی‌دید.
      |
      | نداشتنِ قیمتِ ارزی ⇒ اصلاً `Offer` نساز. نشانه‌گذاریِ نبود، از
