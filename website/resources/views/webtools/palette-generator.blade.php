@php
  ...
  $pgKeys = array_keys($pgSchemes);
@endphp

<style>
  .pg-sub-line{margin-top:9px;font-size:12.5px;color:var(--dim);font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
  .pg-hd{font-size:12.5px;font-weight:600;color:var(--dim);margin:22px 0 10px}
  .pg-schemes{display:flex;flex-wrap:wrap;gap:8px}
  .pg-sch{font-family:var(--font-body);font-size:13px;padding:8px 15px;border-radius:10px;border:1px solid var(--line-2);
    background:var(--surface-2);color:var(--muted);cursor:pointer;transition:border-color .15s,color .15s,background .15s}
  ...
  .pg-strip{display:flex;height:14px;border-radius:99px;overflow:hidden;border:1px solid var(--line-2);margin-top:16px}
  .pg-strip i{flex:1;display:block}

  .pg-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(152px,1fr));gap:12px;margin-top:12px}
  ...
  .pg-chip{display:block;width:100%;height:98px;border:0;padding:0;cursor:pointer;font-family:var(--font-disp);
    font-size:19px;font-weight:700;letter-spacing:.4px}
  .pg-chip:hover{filter:brightness(1.06)}
  .pg-badge{position:absolute;inset-block-start:8px;inset-inline-start:8px;font-style:normal;font-size:10.5px;line-height:1;
    padding:4px 7px;border-radius:99px;border:1px solid currentColor;opacity:.72;pointer-events:none;
    font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
  .pg-lock{position:absolute;inset-block-start:6px;inset-inline-end:6px;width:29px;height:29px;border-radius:9px;
    border:1px solid currentColor;background:transparent;display:grid;place-items:center;cursor:pointer;opacity:.45;transition:opacity .15s}
  ...
  .pg-sub{font-size:11.5px;color:var(--dim);font-family:ui-monospace,SFMono-Regular,Menlo,monospace}
  .pg-cr{display:flex;align-items:center;gap:6px;font-size:11px;color:var(--dim);margin-top:2px}
  .pg-cr b{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;color:var(--muted)}
  .pg-g{font-style:normal;font-size:10px;font-weight:700;border-radius:6px;padding:2px 6px;border:1px solid transparent;white-space:nowrap}
  .pg-g.aaa{color:var(--green);background:rgba(52,211,153,.14);border-color:rgba(52,211,153,.32)}
  .pg-g.aa{color:var(--cyan);background:rgba(34,211,238,.13);border-color:rgba(34,211,238,.3)}
  .pg-g.lg{color:#fbbf24;background:rgba(251,191,36,.13);border-color:rgba(251,191,36,.3)}
  .pg-g.no{color:#ff8080;background:rgba(255,107,107,.12);border-color:rgba(255,107,107,.3)}
  html[data-theme="light"] .pg-g.lg{color:#8a5c04;background:rgba(251,191,36,.2);border-color:rgba(251,191,36,.5)}
  html[data-theme="light"] .pg-g.no{color:#b32424;background:rgba(255,107,107,.15);border-color:rgba(255,107,107,.45)}

  .pg-wrapchip::after{content:'';position:absolute;inset-block-start:50%;inset-inline-start:50%;
    transform:translate(-50%,-50%) scale(.5);width:34px;height:34px;border-radius:50%;background:var(--green);
    opacity:0;transition:.16s;pointer-events:none}
  .pg-wrapchip::before{content:'';position:absolute;inset-block-start:50%;inset-inline-start:50%;z-index:2;
    width:13px;height:7px;margin-block-start:-6px;margin-inline-start:-7px;border-inline-start:2.5px solid #04110b;
    border-block-end:2.5px solid #04110b;transform:rotate(-45deg) scale(.4);opacity:0;transition:.16s;pointer-events:none}
  .pg-sw.pg-copied .pg-wrapchip::after{opacity:1;transform:translate(-50%,-50%) scale(1)}
  .pg-sw.pg-copied .pg-wrapchip::before{opacity:1;transform:rotate(-45deg) scale(1)}

  .pg-lab{display:inline-flex;align-items:center;gap:8px;font-size:13.5px;color:var(--muted)}
  .pg-inp{background:var(--surface-2);border:1px solid var(--line-2);border-radius:9px;color:var(--text);padding:6px 10px;
    font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:13px;width:110px;outline:none}
  .pg-inp:focus{border-color:var(--cyan)}

  .pg-all{display:flex;flex-direction:column;gap:8px}
  .pg-row{display:grid;grid-template-columns:minmax(130px,178px) 1fr;gap:14px;align-items:center;background:var(--surface-2);
    border:1px solid var(--line);border-radius:12px;padding:9px 12px;cursor:pointer;text-align:start;
    font-family:var(--font-body);transition:border-color .15s;width:100%}
  ...
  .pg-rl i{display:block;font-style:normal;font-size:11px;color:var(--dim);
    font-family:ui-monospace,SFMono-Regular,Menlo,monospace;margin-top:3px}
...
<div class="wt-fields">
  ...
  <button type="button" class="btn btn-glass" id="pg-unlockall">
    <svg class="icon"><use href="#i-restore"/></svg> {{ __('ui.wt_pg_unlockall') }}
  </button>
</div>
...
<div class="pg-strip" id="pg-strip"></div>
...
(function () {
  ...
  var T = {
    ...
    cr:      @json(__('ui.wt_pg_cr')),
    large:   @json(__('ui.wt_pg_glarge')),
    fail:    @json(__('ui.wt_pg_gfail')),
    locked:  @json(__('ui.wt_pg_lockedtag'))
  };

  var DEF_ANGLE = { analog: 30, split: 30, tetrad: 90 };

  /* base colour #22D3EE expressed exactly in HSL */
  ...
  /* Chroma form of HSL -> RGB (CSS Color 4). Working in sixths of a turn rather
     than thirds keeps hsl(30,100%,50%) at 128 like a browser does, not 127. */
  function hsl2rgb(h, s, l) {                 /* h degrees, s and l in 0..1 */
  ...
  function i255(n) { return clampN(Math.round(n), 0, 255); }

  /* character-scan parser: no regex, so no backslash can be lost in this file */
  ...
  /* 0.179129 is where contrast against white equals contrast against black */
  function readable(rgb) { return lum(rgb) > 0.179129 ? '#000000' : '#FFFFFF'; }
  function grade(r) {
    if (r >= 7)   return ['aaa', 'AAA'];
    if (r >= 4.5) return ['aa', 'AA'];
    if (r >= 3)   return ['lg', T.large];
    return ['no', T.fail];
  }
  ...
  /* Five lightness stops 15 points apart. The whole 60-point window is slid into
     6..94 instead of clamping each stop, so a near-white base still yields five
     distinct tints rather than three identical ones. The base lightness always
     lands on slot 3 while it sits between 36 and 64. */
  ...
  function buildPal(k, a, useLocks) {
    ...
    for (i = 0; i < offs.length; i++) {
      if (useLocks && st.locks[i]) {
        var lr = parseHex(st.locks[i]);
        out.push({
          hex: st.locks[i], rgb: lr, hsl: rgb2hsl(lr[0], lr[1], lr[2]),
          off: offs[i], locked: true
        });
      } else {
  ...
  function rgbText(c) {
    return 'rgb(' + i255(c.rgb[0]) + ', ' + i255(c.rgb[1]) + ', ' + i255(c.rgb[2]) + ')';
  }
  function badgeText(k, c) {
  ...
  function cleanPrefix(v) {
    ...
    while (out.charAt(out.length - 1) === '-') out = out.slice(0, out.length - 1);
    return out;
  }
  ...
  function exportText(pal) {
    ...
    for (i = 0; i < pal.length; i++) out += pal[i].hex + NL;
    return out.slice(0, out.length - 1);
  }

  /* ---------------- copy helper for the swatch chips ---------------- */
  ...
  function syncInputs(skipHexField) {
    var bh = baseHex(), hh = Math.round(norm(st.h)) % 360;
    $('pg-pick').value = bh.toLowerCase();
    if (!skipHexField) $('pg-hex').value = bh;
    $('pg-h').value = hh;
    $('pg-s').value = Math.round(st.s);
    $('pg-l').value = Math.round(st.l);
    $('pg-hv').textContent = hh;
    $('pg-sv').textContent = Math.round(st.s);
    $('pg-lv').textContent = Math.round(st.l);
    $('pg-av').textContent = st.angle;
    $('pg-a').value = st.angle;
    $('pg-baseinfo').textContent = bh + '  hsl(' + hh + ', ' + Math.round(st.s) + '%, ' + Math.round(st.l) + '%)';
  }

  function renderPalette() {
    var pal = buildPal(st.scheme, st.angle, true), html = '', strip = '', i;
    for (i = 0; i < pal.length; i++) {
      var c = pal[i];
      var txt = readable(c.rgb);
      var cr  = ratio(c.rgb, txt === '#000000' ? [0, 0, 0] : [255, 255, 255]);
      var g   = grade(cr);
      var bg  = badgeText(st.scheme, c);

      strip += '<i style="background:' + c.hex + '"></i>';
      ...
      html += '<div class="pg-sw' + (c.locked ? ' pg-locked' : '') + '" data-i="' + i + '">'
        ...
        +     ' title="' + esc(T.copyone) + '" style="background:' + c.hex + ';color:' + txt + '">' + esc(T.spec) + '</button>'
        +   (bg ? '<em class="pg-badge" dir="ltr" style="color:' + txt + '">' + esc(bg) + '</em>' : '')
        +   '<button type="button" class="pg-lock" data-act="lock" data-i="' + i + '"'
        +     ' aria-pressed="' + (c.locked ? 'true' : 'false') + '"'
        +     ' title="' + esc(c.locked ? T.unlock : T.lock) + '" style="color:' + txt + '">'
        +     '<svg class="icon"><use href="#i-lock"/></svg></button>'
        + '</div>'
        + '<div class="pg-meta">'
        +   '<span class="pg-hex" dir="ltr">' + c.hex + '</span>'
        +   '<span class="pg-sub" dir="ltr">' + hslText(c) + '</span>'
        +   '<span class="pg-sub" dir="ltr">' + rgbText(c) + '</span>'
        +   '<span class="pg-cr">' + esc(T.cr) + ' <b dir="ltr">' + cr.toFixed(2) + '</b>'
        +     '<i class="pg-g ' + g[0] + '">' + esc(g[1]) + '</i></span>'
        + '</div></div>';
    }
    $('pg-strip').innerHTML = strip;
  ...
  function renderAll() {
    ...
    for (n = 0; n < KEYS.length; n++) {
      ...
      html += '<button type="button" class="pg-row' + (k === st.scheme ? ' pg-on' : '') + '" data-k="' + k + '">'
        + '<span class="pg-rl">' + esc(SCH[k]) + '<i dir="ltr">' + geometryText(k, a) + '</i></span>'
  ...
  function pickScheme(k) {
    if (!SCH[k]) return;
    st.scheme = k;
    st.locks = {};                              /* slot count differs per scheme */
    if (DEF_ANGLE[k] !== undefined) st.angle = DEF_ANGLE[k];
  ...
  $('pg-hex').addEventListener('input', function () {
    ...
    if (!rgb) { $('pg-err').textContent = T.bad; $('pg-err').style.display = 'block'; return; }
    $('pg-err').style.display = 'none';
