    ...
    <div class="auth-shell" style="grid-template-columns:1fr">
      <div class="auth-main">
        ...
        @if($payment)
          <div class="pay-rows">
            @if($payment->ref_id)
              {{-- 🔴 برچسبِ اسنپ‌پی فرق می‌کند و خواستهٔ صریحِ خودشان است:
                   «پس از پرداخت موفق لازم هست که شماره تراکنش آیدی (مشترک و
                   یونیک بین پذیرنده و اسنپ‌پی) به کاربر نمایش داده شود.»
                   همین شماره است که پشتیبانیِ دو طرف با آن پیگیری می‌کند،
                   پس «شمارهٔ پیگیری بانک» نامیدنش گمراه‌کننده است. --}}
              <div class="pay-row hero">
                <span>{{ ($snapppay ?? null) ? __('ui.pres_ref_snapppay') : __('ui.pres_ref') }}</span>
