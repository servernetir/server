...
{{-- 🔴 صورت‌حسابِ ساعتی و پایانِ اعتبار — در هر حالتی (ساخت، روشن، خاموش) --}}
@if($service->isHourly())
  @include('account.partials.hourly-billing', ['service' => $service])
@endif
...
<section class="pnl-sec">
  <div class="pnl-sec-h"><h2>{{ __('ui.cxp_egress_h') }}</h2></div>
  <div class="pnl-sec-b">
    ...
    <form method="post" action="{{ lroute('account.cloud.exit-country', $service) }}"
      ...
      <button type="submit" class="pnl-btn">{{ __('ui.cxp_egress_btn') }}</button>
    </form>
  </div>
</section>
@endif
...
<section class="pnl-sec" id="tunnel">
  <div class="pnl-sec-h"><h2>{{ __('ui.cxp_tun_h') }}</h2></div>
  <div class="pnl-sec-b">
      ...
      <div class="pnl-sec" style="border-color:var(--ok-line);margin:0 0 16px">
        <div class="pnl-sec-b">
          <p style="margin:0 0 10px;font-size:13px;color:var(--ok)">
            «{{ $tunnelIssued['name'] }}» ({{ $tunnelIssued['ip'] }}) {{ __('ui.cxp_tun_created') }}
          </p>
          ...
          <div style="position:relative">
            ...
            <button type="button" class="pnl-btn" data-tn-copy="tn-cmd" style="margin-top:8px">{{ __('ui.cxp_copy_cmd') }}</button>
          </div>

          <p style="margin:18px 0 6px;font-size:12.5px;color:var(--dim)">
            {{ __('ui.cxp_step2_a') }} <span dir="ltr">{{ $tunnelIssued['file'] }}</span>
            {{ __('ui.cxp_step2_b') }}
          </p>
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:0 0 8px">
            <button type="button" class="pnl-btn" id="tn-fmt-new" style="opacity:1">{{ __('ui.cxp_fmt_new') }}</button>
            <button type="button" class="pnl-btn" id="tn-fmt-old" style="opacity:.55">{{ __('ui.cxp_fmt_old') }}</button>
          </div>
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px">
            <button type="button" class="pnl-btn" data-tn-copy="tn-cfg">{{ __('ui.cxp_copy_cfg') }}</button>
            <button type="button" class="pnl-btn" id="tn-dl">{{ __('ui.cxp_dl_file') }}</button>
          </div>
    ...
    @if(session('tunnel_removed'))
      <div class="pnl-sec" style="border-color:var(--danger-line);margin:0 0 16px">
        <div class="pnl-sec-b">
          ...
          <button type="button" class="pnl-btn" data-tn-copy="tn-del" style="margin-top:8px">{{ __('ui.cxp_copy_cmd') }}</button>
        </div>
      </div>
    @endif
    ...
    @if($tunnelNextIp)
      <form method="post" action="{{ lroute('account.cloud.tunnel.issue', $service) }}"
        ...
        <button type="submit" class="pnl-btn">{{ __('ui.cxp_create_btn') }}</button>
      </form>
    @else
      <p style="margin:0;font-size:12.5px;color:var(--dim)">
        {{ __('ui.cxp_range_full') }}
      </p>
    @endif
