...
<div class="ad-panel">
  ...
  @else
    ...
    <div style="overflow-x:auto">
    {{-- 🔴 `data-no-enhance` عمدی است و **باید** بمانَد.

         این جدول فیلترِ غنیِ خودش را دارد و ردیف‌ها را با کلاسِ `.ad-hide`
         پنهان می‌کند، در حالی که ارتقای عمومیِ جدول‌ها با `style.display`
         پنهان می‌کند. دو سازوکار روی یک ردیف یعنی ردیفی که مدیر **نمی‌بیند**
         ولی `visible(tr)` هنوز «دیده‌شده» می‌شمارَدش — و آن ردیف در انتخابِ
         گروهی می‌افتد. دقیقاً همان خرابیِ «۴۰۰ ردیف را می‌بندم در حالی که ۱۲
         تا می‌بینم» که قرارداد پایین‌تر برای جلوگیری از آن نوشته شده. --}}
    <table class="ad-table" id="cl-table" data-no-enhance>
      <thead><tr>
...
{{--
  ...
  ۱) بلوکِ خامِ زیر لازم است. یک `{{` یا یک `@word` تصادفی در جاوااسکریپت را Blade
