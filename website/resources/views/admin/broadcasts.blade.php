...
<div class="ad-panel">
  ...
  <p style="padding:0 18px;color:#96a3ba;font-size:13.5px;line-height:1.9">
    اعلان هم‌زمان از <b style="color:#e7edf7">پیامک</b> و <b style="color:#e7edf7">بله</b> برای مشتری می‌رود.
  ...
  <form method="post" action="/admin/broadcasts" style="padding:16px" id="bc-form">
    ...
    <div id="bc-one" style="margin-bottom:14px;{{ $preselect ? '' : 'display:none' }}">
      <label style="font-size:13px;color:#96a3ba;display:block;margin-bottom:6px">شناسهٔ مشتری (id عددی)</label>
      <input type="number" name="customer_id" value="{{ $preselect }}" dir="ltr"
             style="background:#0f1522;border:1px solid #1e2637;border-radius:8px;color:#e7edf7;padding:8px 12px;font:inherit;width:200px;text-align:left">
      <small style="color:#5f6c82;display:block;margin-top:5px">از پروندهٔ هر مشتری دکمهٔ «ارسال اعلان» این را خودکار پر می‌کند.</small>
    </div>

    <div style="margin-bottom:14px">
      <label style="font-size:13px;color:#96a3ba;display:block;margin-bottom:6px">عنوان (اختیاری)</label>
      <input type="text" name="title" value="{{ old('title') }}" maxlength="120"
             style="width:100%;background:#0f1522;border:1px solid #1e2637;border-radius:8px;color:#e7edf7;padding:9px 12px;font:inherit">
    </div>

    <div style="margin-bottom:14px">
      <label style="font-size:13px;color:#96a3ba;display:block;margin-bottom:6px">متن پیام</label>
      <textarea name="body" required maxlength="1000" rows="4"
                style="width:100%;background:#0f1522;border:1px solid #1e2637;border-radius:8px;color:#e7edf7;padding:10px 12px;font:inherit;line-height:1.9;resize:vertical">{{ old('body') }}</textarea>
    </div>
...
<div class="ad-panel">
  ...
  @if($history->isEmpty())
    <p style="padding:16px;color:#5f6c82">هنوز اعلانی ارسال نشده.</p>
  @else
...
.bc-aud label{ display:flex; flex-direction:column; gap:4px; padding:12px; background:#0f1522; border:1px solid #1e2637; border-radius:10px; cursor:pointer; position:relative }
.bc-aud label:has(input:checked){ border-color:#22d3ee; background:rgba(34,211,238,.06) }
.bc-aud input{ position:absolute; opacity:0 }
.bc-aud span{ font-size:13.5px; color:#e7edf7 }
.bc-aud i{ font-style:normal; font-size:12px; color:#5f6c82; font-variant-numeric:tabular-nums }
