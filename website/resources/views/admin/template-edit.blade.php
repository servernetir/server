...
<div class="ad-panel">
  ...
  <form method="post" action="/admin/templates/{{ $t->id }}" style="padding:0 18px 18px" id="tpl-form">
    ...
    {{-- ── پیامک ──
         🔴 قبلاً این‌جا می‌نوشت «اگر الگو نبود، متن کوتاهِ پایین به‌عنوان پیام
         آزاد می‌رود». آن جمله **دروغ** بود: درایورِ فعال (رلهٔ n8n) متنِ آزاد را
         عمداً نمی‌فرستد، چون به هیچ الگویی نگاشت نمی‌شود. مدیر متنِ تعلیق را
         ویرایش می‌کرد با این خیال که پیامک می‌رود، و مشتریِ بدهکار هیچ پیامکی
         نمی‌گرفت. حالا وضعیت از سیمِ واقعی خوانده می‌شود، نه از ستونِ جدول. --}}
    @php $hasSms = in_array('sms', $t->wiredChannels()); @endphp
    <div class="ad-field">
      <label>پیامک</label>
      <div style="background:var(--surface2);border:1px solid var(--line2);border-radius:11px;padding:12px 14px;font-size:12.5px;color:var(--muted);line-height:1.9">
        @if($hasSms)
          این پیام با الگوی <b dir="ltr">{{ $t->sms_event ?: $t->key }}</b> فرستاده می‌شود.
          <br>متن الگو در پنل اپراتور پیامک نگهداری و تأیید می‌شود، پس از این‌جا ویرایش نمی‌شود؛
          ما فقط کد الگو و متغیرها را می‌فرستیم.
        @else
          <b style="color:#fbbf24">برای این رویداد پیامکی ارسال نمی‌شود.</b>
          <br>الگویی در پنل اپراتور برایش ساخته نشده، و متنِ آزاد هم فرستاده نمی‌شود
          (اپراتور پیامِ بی‌الگو را رد می‌کند). این رویداد فقط از بله و ایمیل می‌رود.
          <br>اگر باید پیامک بدهد، اول در پنل اپراتور الگو بسازید و بعد کدش را به
          <code dir="ltr">SignedRelaySender::TEMPLATES</code> و رجیستری n8n اضافه کنید.
        @endif
      </div>
    </div>
    ...
    <div class="ad-field">
      <label>متن کوتاه — بله و اعلان پنل</label>
