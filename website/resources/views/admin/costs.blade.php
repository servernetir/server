        ...
        <tr>
          ...
          <td>
            <input type="number" name="amount[{{ $cost->id }}]" value="{{ $cost->amount }}" min="0" step="1"
                   dir="ltr" style="width:170px;background:var(--surface2);border:1px solid var(--line);border-radius:8px;color:var(--text);padding:7px 10px;font:inherit;text-align:left">
          </td>
          <td>
            <input type="text" name="note[{{ $cost->id }}]" value="{{ $cost->note }}" placeholder="—"
                   style="width:100%;background:var(--surface2);border:1px solid var(--line);border-radius:8px;color:var(--muted);padding:7px 10px;font:inherit">
          </td>
          <td class="ad-row-act">
...
<div class="ad-panel">
  ...
  <form method="post" action="/admin/costs/add" style="padding:16px;display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap">
    @csrf
    <label style="display:flex;flex-direction:column;gap:5px;font-size:13px;color:var(--muted)">عنوان
      <input type="text" name="label" required placeholder="مثلاً لایسنس ماهانه cPanel" style="background:var(--surface2);border:1px solid var(--line);border-radius:8px;color:var(--text);padding:8px 12px;font:inherit;min-width:240px"></label>
    <label style="display:flex;flex-direction:column;gap:5px;font-size:13px;color:var(--muted)">مبلغ (تومان)
      <input type="number" name="amount" required min="0" step="1" dir="ltr" style="background:var(--surface2);border:1px solid var(--line);border-radius:8px;color:var(--text);padding:8px 12px;font:inherit;width:160px;text-align:left"></label>
    <label style="display:flex;flex-direction:column;gap:5px;font-size:13px;color:var(--muted)">توضیح
      <input type="text" name="note" style="background:var(--surface2);border:1px solid var(--line);border-radius:8px;color:var(--text);padding:8px 12px;font:inherit;min-width:200px"></label>
