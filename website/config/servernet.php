...
return [
        ...
        'vps' => [
            ...
            'groups' => [
                ['fa' => 'سرور مجازی', 'en' => 'Virtual servers', 'tr' => 'Sanal sunucular', 'items' => [
                    ...
                    // ⚠️ سرورِ گرافیکی عمداً **خطِ محصولِ خودش** است و زیرِ
                    // `/vps/` نیست (آن مسیر ۳۰۱ می‌خورد و وعدهٔ ماشینِ پایدار
                    // می‌دهد)، ولی در منو کنارِ ساعتی می‌نشیند چون مشتری هر دو
                    // را با یک نیت می‌جوید: «محاسبه، ساعتی، بی‌قرارداد».
                    ['route' => ['gpu', []], 'new' => true,
    ...
    */
    ...
    /*
    | ═══ میزبانِ ایران (پشتِ NAT) ═══
    |
    | 🔴 این کلید تا امروز **وجود نداشت** در حالی که چهار جای کد به آن تکیه
    | می‌کردند: `PullController::portForwards`، `ExitInfraController`،
    | `CloudInstance::publicHost()` و صفحهٔ تنظیمات. هر چهار `config(...)` را
    | با پیش‌فرضِ خالی صدا می‌زدند، پس همیشه `''` می‌گرفتند و کسی نمی‌فهمید که
    | پشتوانه‌ای در کار نیست — تا روزی که تنظیمِ `public_ip` هم خالی بود و
    | آدرسِ مشتری «—» شد.
    |
    | ⚠️ ترتیبِ اولویت عمدی است: تنظیماتِ پنل (که مدیر عوض می‌کند) بر این
    | مقدار می‌چربد. این فقط پشتوانهٔ استقرار است، نه منبعِ اصلی.
    */
    'exit' => [
        'public_ip' => env('SERVERNET_EXIT_PUBLIC_IP', ''),
    ],
