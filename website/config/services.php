...
return [
    ...
    */
    'openprovider' => [
        ...
        */
        ...
        /*
        |----------------------------------------------------------------------
        | مالکِ ثابتِ شرکت — مشخصاتی که به رجیسترار می‌رود
        |----------------------------------------------------------------------
        |
        | 🔴 خواستهٔ کارفرما: «برای کلِ دامنه‌های ثبتی توسطِ API، مشخصاتِ خودم را
        | بگذار.» این ریشهٔ صفِ دستی را می‌خشکاند: تا امروز هر ثبت به کامل‌بودنِ
        | پروفایلِ **مشتری** بند بود و اکثر مشتری‌ها نشانی و تلفن نداده‌اند.
        |
        | ⚠️ پیامدِ واقعی که باید بدانید: با این کار، در WHOIS و نزدِ رجیستری
        | **شرکت** مالکِ ثبت‌شده است، نه مشتری. این همان کاری است که بیشترِ
        | نمایندگی‌ها می‌کنند و کاملاً رایج است، ولی دو اثر دارد:
        |   • انتقالِ دامنه به رجیسترارِ دیگر از مسیرِ ما می‌گذرد
        |   • در اختلافِ مالکیت، سندِ رجیستری به نامِ ماست
        | مالکیتِ واقعیِ مشتری در جدولِ `domains` ما ثبت و نگه داشته می‌شود.
        |
        | ⚠️ اگر خالی بماند، رفتار **دقیقاً مثلِ قبل** است و از پروفایلِ مشتری
        ...
        */
        ...
        /*
        | آستانهٔ هشدارِ موجودیِ حساب نزدِ رجیسترار (به ارزِ حساب — معمولاً یورو).
        | پایینِ این عدد، چکِ سلامتِ `domain_balance` قرمز می‌شود تا **پیش از**
        | شکستِ ثبت/تمدید خبر برسد، نه بعد از انباشتِ صفِ دستی.
        */
        'min_balance' => (float) env('OPENPROVIDER_MIN_BALANCE', 10),
    ],

    /*
    |----------------------------------------------------------------------
    | zone خودکارِ دامنه — DNS پشتِ نیم‌سرورهای پیش‌فرض
    |----------------------------------------------------------------------
    | ns1/ns2.servernet.cloud به سرورِ cPanelِ core اشاره می‌کنند و BIND همان
    | سرور پاسخ‌گوست؛ پس هر دامنه‌ای که با NSِ پیش‌فرض ثبت شود باید همان‌جا
    | zone بگیرد (WHM adddns) وگرنه به هیچ‌جا resolve نمی‌شود.
    | هر دو خالی می‌توانند بمانند: `DomainZoneProvisioner` خودش سرورِ WHMای
    | را پیدا می‌کند که hostnameاش با IPِ ns1 یکی است و IPِ رکوردِ A را هم از
    | همان می‌گیرد. تنظیماتِ هم‌نام (`domain_zone_server_id` / `domain_zone_ip`)
    | در جدولِ settings بر اینها مقدم‌اند.
    */
    'domain_zone' => [
        'server_id' => (int) env('DOMAIN_ZONE_SERVER_ID', 0),
        'ip'        => env('DOMAIN_ZONE_IP'),
    ],
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | نرخِ ارز — کلیدِ خاموشیِ دریافتِ زنده
    |--------------------------------------------------------------------------
    | خاموش که باشد، `ExchangeRate::refresh()` هیچ تماسِ بیرونی نمی‌زند و نرخِ
    | ذخیره‌شده دست‌نخورده می‌مانَد. دو کاربرد:
    |
    |  • **تست**: بی‌این، سناریوی «نرخ سرد» ساختنی نبود — کشِ خالی خودش با یک
    |    اسکرپِ زنده پر می‌شد و سبزیِ سوئیت به در دسترس بودنِ اینترنت گره
    |    می‌خورد. در phpunit.xml خاموش است.
    |  • **پروداکشن**: اگر منبعِ نرخ خراب یا دستکاری شود، این تنها راهِ فریزکردنِ
    |    قیمتِ کلِ سایت بدونِ ویرایشِ کد است.
    */
    'exchange' => [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | گوگل تگ منیجر و گوگل آنالیتیکس ۴ (GTM & GA4)
    |--------------------------------------------------------------------------
    | شناسهٔ کانتینر Google Tag Manager وب‌سایت سرورنت.
