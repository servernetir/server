...
return [
    ...
    */
    ...
    /*
    |----------------------------------------------------------------------
    | زحل — احراز هویت ایرانی (شاهکار، استعلام هویت، کارت بانکی)
    |----------------------------------------------------------------------
    | مستندات زحل پشت داشبورد است، پس مسیرها و نگاشت فیلدها از اینجا خوانده
    | می‌شوند. وقتی مستندات واقعی را دیدید همین‌جا پر کنید — کد تغییر نمی‌کند.
    */
    'zohal' => [
        'base_url'   => env('ZOHAL_BASE_URL', 'https://api.zohal.io'),
        'token'      => env('ZOHAL_TOKEN'),
        'auth_style' => env('ZOHAL_AUTH_STYLE', 'bearer'),
        'endpoints' => [
            'shahkar'  => ['path' => env('ZOHAL_PATH_SHAHKAR', ''),  'method' => 'POST', 'fields' => []],
            'identity' => ['path' => env('ZOHAL_PATH_IDENTITY', ''), 'method' => 'POST', 'fields' => []],
            'card'     => ['path' => env('ZOHAL_PATH_CARD', ''),     'method' => 'POST', 'fields' => []],
        ],
        // نگاشت مسیر فیلدها در پاسخ. خالی = تشخیص خودکار مسیرهای متداول.
        'map' => [],
    ],
